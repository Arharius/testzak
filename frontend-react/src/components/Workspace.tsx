          ...
          {uiPhase === 'working' && (
            <>
              <button
                ...
                title={paymentRequired ? 'Trial завершён' : !canUseAiAssist ? 'Требуется вход' : 'Найти характеристики в открытых источниках по модели'}
              >
                {internetSearching ? '⏳ Ищу...' : paymentRequired ? '🔒 Закрыто' : '🌐 Найти в интернете'}
              </button>
              <button
                type="button"
                onClick={() => void searchZakupki()}
                disabled={paymentRequired || eisSearching || !canUseAiAssist || publicationAutopilotRunning}
                title={paymentRequired ? 'Trial завершён' : !canUseAiAssist ? 'Требуется вход' : 'Найти похожие закупки в ЕИС'}
              >
                {eisSearching ? '⏳ Ищу в ЕИС...' : paymentRequired ? '🔒 Закрыто' : '🏛️ Найти в ЕИС'}
              </button>
              <button type="button" onClick={resetWorkspaceDraft} title="Сбросить все данные и начать заново">
                ↺ Сбросить
              </button>
            </>
          )}
