...
Пользователь хочет закупить товар. Описание: "${row.model}"
Количество: ${row.qty} шт.

ТВОЯ ЗАДАЧА — определить:
1. Правильный код ОКПД2 (до 3 знаков после точки минимум)
2. Полное наименование ОКПД2
3. Код КТРУ (если есть в каталоге КТРУ)
4. Национальный режим: "pp878" (радиоэлектроника РЭПР), "pp1236" (ПО), "pp616" (промтовары), "none" (без ограничений)
5. Полный набор технических характеристик (не менее 15-25), сгруппированных по разделам

ЗАПРЕЩЕНО (нарушает ст. 33 44-ФЗ):
- Торговые марки, производители, артикулы, part numbers, SKU
- Слова «рекомендуется», «желательно» — только «должен», «не менее», «не более»
- Конкретные технологии без «или эквивалент» (ч. 3 ст. 33 44-ФЗ)

ОБЯЗАТЕЛЬНЫЕ правила:
- Числовые значения: «не менее X» или «не более X»
- Единицы измерения на русском языке (мм, кг, Вт, л, шт, м², °C)
- Вес/масса: «не более X кг»
- Размеры: «не менее/не более X мм»
- Материалы/технологии: с добавлением «или эквивалент»

Характеристики должны быть РЕАЛЬНЫМИ, КОНКРЕТНЫМИ, соответствовать именно этому товару.
НЕ выдумывай характеристики — используй свои знания о реальных параметрах данного типа товаров.
Уровень детализации должен соответствовать реальным ТЗ из ЕИС (zakupki.gov.ru).
...
Ответ СТРОГО в JSON:
{
  ...
  "specs": [
    {"group":"Общие сведения","name":"Наименование","value":"описание товара","unit":""},
    {"group":"Технические характеристики","name":"Параметр","value":"не менее/не более X","unit":"ед.изм."}
  ]
}`;
  }
...
ОБЯЗАТЕЛЬНЫЕ параметры для включения (список ниже — МИНИМУМ, добавь больше если применимо):
${hint}

ВАЖНО: Для ПО необходимо не менее 25-40 характеристик, сгруппированных по разделам (Общие сведения, Функциональные возможности, Безопасность, Совместимость, Сертификация и лицензирование).
Для оборудования — не менее 15-25 характеристик. Уровень детализации должен соответствовать реальным ТЗ из ЕИС (zakupki.gov.ru).
