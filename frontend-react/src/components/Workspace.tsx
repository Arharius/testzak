...
function buildPrompt(
  ...
  const systemPrompt = `Ты — ведущий эксперт по формированию технических заданий для государственных закупок РФ (${law}), специализирующийся на ИТ-оборудовании и программном обеспечении.
Твоя задача — сформировать ПОЛНЫЙ, ДЕТАЛЬНЫЙ, ЮРИДИЧЕСКИ БЕЗУПРЕЧНЫЙ набор технических характеристик для закупочной документации.

═══════════════════════════════════════════════════════════
ПРАВИЛО «ДВОЙНОГО ЭКВИВАЛЕНТА» (ключевое требование ФАС):
═══════════════════════════════════════════════════════════
Каждая числовая характеристика ДОЛЖНА допускать продукцию минимум ДВУХ разных производителей.
- НЕ пиши точные значения, которые есть только у одного вендора
- Используй диапазоны: «не менее X» или «не более X», позволяющие конкуренцию
- Проверь мысленно: подходит ли это ТЗ хотя бы двум известным производителям?
- Пример правильно: «не менее 16 ГБ» (подходит Samsung, SK Hynix, Micron)
- Пример неправильно: «ровно 16384 МБ DDR4-3200 Samsung» (один производитель)

═══════════════════════════════════════════════════════════
КРИТИЧЕСКИЕ ПРАВИЛА (нарушение = отклонение ФАС / иск):
═══════════════════════════════════════════════════════════
1. ЗАПРЕЩЕНЫ: торговые марки, бренды, производители, артикулы, SKU, конкретные модели в значениях
2. ЗАПРЕЩЕНЫ: слова «рекомендуется», «желательно», «опционально» — ТОЛЬКО «должен», «не менее», «не более»
3. Конкретные технологии/стандарты: ТОЛЬКО с «или эквивалент» (ч. 3 ст. 33 44-ФЗ)
4. Числовые параметры ТОЛЬКО через «не менее X» / «не более X» — НИКОГДА просто голое число
5. Единицы измерения ТОЛЬКО на русском: ГГц, МГц, ГБ, МБ, ТБ, мм, кг, Вт, дБА, °C, шт, мес
6. Каждый параметр ДОЛЖЕН быть измеримым/проверяемым при приёмке товара

═══════════════════════════════════════════════════════════
ОБЯЗАТЕЛЬНЫЕ КАТЕГОРИИ ХАРАКТЕРИСТИК (для аппаратного ИТ):
═══════════════════════════════════════════════════════════
А. ПРОИЗВОДИТЕЛЬНОСТЬ: процессор (частота, ядра, кэш), RAM, хранилище (скорость чтения/записи)
Б. ИНТЕРФЕЙСЫ И СВЯЗЬ: USB (тип и версия), DisplayPort/HDMI (версия), RJ-45 (скорость), Wi-Fi (стандарт IEEE 802.11), Bluetooth (версия)
В. ПИТАНИЕ И ЭНЕРГОЭФФЕКТИВНОСТЬ: TDP (Вт), КПД блока питания (не менее N%), энергопотребление в режиме простоя (Вт), класс энергоэффективности
Г. НАДЁЖНОСТЬ: MTBF (не менее N часов), гарантийный срок (мес), рабочая температура (°C), влажность (%RH без конденсации)
Д. ФИЗИЧЕСКИЕ ПАРАМЕТРЫ: габариты ШxГxВ (мм), масса (кг), форм-фактор
Е. БЕЗОПАСНОСТЬ И СЕРТИФИКАЦИЯ: соответствие ГОСТ, класс защиты IP (при наличии), совместимость с российскими ОС
...
- Поле "group": группируй логически (Процессор, Оперативная память, Хранилище, Интерфейсы, Питание, Надёжность, Физические характеристики, Программное обеспечение, Сертификация)
    ...
    <section className="panel">
      ...
      <div className="workspace-progress-strip">
        ...
        <div className="workspace-progress-actions">
          {hasPublicationBaseline ? (
            <>
              <span className="workspace-mini-chip is-block">Block: {visibleReadinessBlockers}</span>
              <span className="workspace-mini-chip is-warn">Warn: {visibleReadinessWarnings}</span>
            </>
          ) : (
      ...
      <div className="workspace-side-card workspace-composer-card">
        ...
        <div className="workspace-composer-stack">
          ...
          <div className="workspace-inline-note">
            Чтобы вручную завести свой товар, нажмите `📦 Свой товар` или выберите `⭐ Свой товар (ручной ввод)`, затем просто напишите описание вроде `телевизор 55 дюймов`, `масло для шредера 350 мл`, `DVD-R`, `CD-RW`, `металлодетектор`, `монтаж ЛВС`.
          </div>
          <div className="workspace-chip-row workspace-chip-row--dense">
