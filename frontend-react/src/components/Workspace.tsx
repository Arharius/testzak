...
interface GoodsRow {
  ...
  verification?: import('../utils/verify').VerifyResult;
  verifying?: boolean;
...
function isUniversalGoodsType(type: string): boolean {
  return type === 'otherGoods' || type === 'otherService' || type === 'otherWork';
}
...
function isUniversalWorkType(type: string): boolean {
  return type === 'otherWork';
}
...
const specHintsMap: Record<string, string> = {
    // ═══════ ОБОРУДОВАНИЕ ═══════
    pc: [
      '⚠️ ОФИСНЫЙ ПК ДЛЯ ГОСЗАКУПКИ — ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА:',
      '  1. Форм-фактор: НЕ фиксировать как единственный тип. Писать: "Tower, SFF или USFF — на усмотрение поставщика".',
      '  2. Wi-Fi и Bluetooth: НЕ включать по умолчанию (в госорганах часто запрещены по ИБ).',
      '     Включать ТОЛЬКО если заказчик явно указал необходимость.',
      '  3. ОС: Astra Linux, ALT Linux, РЕД ОС или эквивалентная из реестра Минцифры.',
      '',
      '⚠️ ДЕФОЛТНЫЕ ЗНАЧЕНИЯ (использовать если конкретная модель не указана):',
      '  Процессор: ядра не менее 4 шт. | потоки не менее 8 шт. | частота базовая не менее 2.4 ГГц | частота турбо не менее 3.2 ГГц | кэш L3 не менее 8 МБ | TDP не более 65 Вт | архитектура x86-64',
      '  Память: DDR4 или DDR5 | объём не менее 8 ГБ | частота не менее 2400 МГц | слотов не менее 2 шт.',
      '  Накопитель: SSD | интерфейс NVMe M.2 или SATA | объём не менее 256 ГБ',
      '  Видео: интегрированный или дискретный | выходов не менее 2 шт.',
      '  Интерфейсы (зад.): USB 3.2 Gen1+ не менее 4 шт. | HDMI не менее 1 шт. | DisplayPort не менее 1 шт. | Ethernet 1 Гбит/с RJ-45 | аудио 3.5 мм',
      '  Питание БП: не менее 90 Вт | потребление в работе не более 65 Вт | в ожидании не более 5 Вт',
      '  Шум: не более 35 дБА | Температура: +5…+35 °C | Влажность: 20–80% без конденсации',
      '  Гарантия: не менее 36 месяцев',
      '',
      '- Корпус (тип: Tower / SFF / USFF — на усмотрение поставщика; цвет, материал)',
      '- Процессор (архитектура x86-64; кол-во ядер не менее 4; кол-во потоков не менее 8; частота базовая не менее 2.4 ГГц; частота турбо не менее 3.2 ГГц; кэш L3 не менее 8 МБ; TDP не более 65 Вт)',
      '- Оперативная память (тип DDR4 или DDR5; объём не менее 8 ГБ; частота не менее 2400 МГц; слотов не менее 2 шт.; макс. объём не менее 32 ГБ)',
      '- Накопитель (тип SSD; интерфейс NVMe M.2 или SATA; объём не менее 256 ГБ)',
      '- Видеосистема (тип: интегрированная или дискретная; кол-во видеовыходов не менее 2 шт.)',
      '- Интерфейсы задней панели (USB 3.2 Gen1+ не менее 4 шт.; HDMI не менее 1 шт.; DisplayPort не менее 1 шт.; Ethernet RJ-45 1 Гбит/с не менее 1 шт.; аудио 3.5 мм)',
      '- Интерфейсы передней панели (USB не менее 2 шт.; аудио 3.5 мм — при наличии)',
      '- Слоты расширения (PCIe x16, PCIe x1 — при наличии)',
      '- Блок питания (мощность не менее 90 Вт; сертификат 80 PLUS или эквивалент)',
      '- Потребляемая мощность в режиме работы (не более 65 Вт)',
      '- Потребляемая мощность в режиме ожидания (не более 5 Вт)',
      '- Уровень шума (не более 35 дБА)',
      '- Рабочая температура (от +5 до +35 °C)',
      '- Влажность без конденсации (от 20% до 80%)',
      '- Габаритные размеры (ШxВxГ, мм)',
      '- Масса (не более, кг)',
      '- Совместимость с ОС (Astra Linux, ALT Linux, РЕД ОС или эквивалентная из реестра Минцифры)',
      '- Предустановленная ОС (наличие, наименование)',
      '- Гарантия производителя (не менее 36 месяцев)',
    ].join('\n'),
    ...
    switch: [
      ...
      '- Таблица MAC-адресов (не менее 8192 записей)',
      '- Количество VLAN (не менее)',
      '- Протоколы (IEEE 802.3, IEEE 802.1Q VLAN, STP/RSTP/MSTP, LACP, IGMP, SNMP v1/v2c/v3, QoS)',
      '- Списки доступа (ACL)',
      '- Стекирование (при наличии)',
      '- Консольный порт (RJ-45/USB)',
      '- Монтаж (rack 19\"/desktop)',
      '- Потребляемая мощность (не более, Вт)',
      '- Совместимость программного обеспечения управления с ОС: Astra Linux, РЕД ОС или эквивалентными отечественными ОС (для управляемых коммутаторов)',
    ...
    ups: [
      '⚠️ ОФИСНЫЙ ИБП ДЛЯ ГОСЗАКУПКИ — ДЕФОЛТНЫЕ ЗНАЧЕНИЯ:',
      '  Тип: линейно-интерактивный (для рабочих станций); онлайн с двойным преобразованием (для серверов)',
      '  Мощность: не менее 650 ВА / 360 Вт (рабочая станция); не менее 1000 ВА / 600 Вт (сервер)',
      '  Вход: 145–295 В однофазный 50 Гц | Выход: 220–230 В ±5%',
      '  Выходной сигнал: ступенчатая аппроксимация синусоиды (линейно-интерактивный)',
      '  Автономия при 50% нагрузке: не менее 5 мин | при 100%: не менее 3 мин',
      '  Розетки: не менее 4 шт. типа IEC 320 C13 или Schuko',
      '  Аккумулятор: свинцово-кислотный необслуживаемый (AGM)',
      '  Управление: USB Type-B | Гарантия: не менее 24 мес',
      '',
      '- Тип (линейно-интерактивный / онлайн с двойным преобразованием)',
      '- Мощность (не менее, ВА и Вт)',
      '- Входное напряжение (диапазон, В; частота 50 Гц, однофазный)',
      '- Выходное напряжение (В, допустимое отклонение ±%)',
      '- Форма выходного сигнала (чистая синусоида / ступенчатая аппроксимация)',
      '- Время автономной работы при 50% нагрузке (не менее, мин)',
      '- Время автономной работы при 100% нагрузке (не менее, мин)',
      '- Кол-во выходных розеток (не менее, тип IEC320 C13/C19 или Schuko)',
      '- Защита от перегрузки и короткого замыкания (наличие)',
      '- Интерфейс управления (USB Type-B / RS-232 / Ethernet-SNMP-карта)',
      '- Индикация (ЖК-дисплей или светодиодная)',
      '- Тип аккумулятора (свинцово-кислотный необслуживаемый AGM)',
      '- Возможность замены аккумулятора без выключения (горячая замена — при наличии)',
      '- Форм-фактор (настольный / rack; при rack — высота в юнитах U)',
      '- Габариты (ШxГxВ, мм), масса (не более, кг)',
      '- Гарантия производителя (не менее, мес)',
    ].join('\n'),
    ...
    office: [
      ...
      '- Поддерживаемые ОС: Astra Linux, ALT Linux, РЕД ОС или эквивалентные ОС из реестра Минцифры',
      '- Работа без подключения к сети Интернет: обязательна (офлайн-активация и использование)',
      ...
      '--- ГРУППА: Сертификация и лицензирование ---',
      '- Наличие в Едином реестре российского ПО Минцифры России',
      '- Тип лицензии (бессрочная/подписка, на рабочее место)',
    ...
    antivirus: [
      ...
      '- Периодичность обновления антивирусных баз: не реже 1 раза в 24 часа (рекомендуется не реже 1 раза в час)',
      '- Возможность работы без подключения к Интернету: обязательна (локальный сервер обновлений / офлайн-обновление)',
      '--- ГРУППА: Совместимость ---',
      '- Поддерживаемые ОС рабочих станций: Windows 10/11, Astra Linux, ALT Linux, РЕД ОС или эквивалентные',
      '- Поддерживаемые ОС серверов: Windows Server, Astra Linux Server, ALT Server или эквивалентные',
    ...
    keyboard: [
      '⚠️ ОФИСНЫЙ СТАНДАРТ ДЛЯ ГОСЗАКУПКИ:',
      '  Тип: проводная USB (мембранная или ножничная) | Раскладка: русская + латинская, QWERTY | Клавиш: не менее 104 | Кабель: не менее 1.5 м | Гарантия: не менее 12 месяцев',
      '  НЕ включать без явного запроса: подсветку клавиш, Bluetooth, беспроводной режим RF.',
      '',
      '- Тип переключателей (мембранный / ножничный / механический)',
      '- Интерфейс подключения (USB — для проводной; RF 2.4 ГГц — для беспроводной)',
      '- Раскладка (русская + латинская, QWERTY)',
      '- Количество клавиш (не менее, шт.)',
      '- Цифровой блок (наличие)',
      '- Ресурс клавиш (не менее, млн нажатий)',
      '- Длина кабеля (не менее, м) — для проводной',
      '- Совместимость с ОС (Astra Linux, Windows или эквивалентными)',
      '- Цвет корпуса',
      '- Масса (не более, г)',
      '- Гарантия производителя (не менее, мес)',
    ].join('\n'),
    mouse: [
      '⚠️ ОФИСНЫЙ СТАНДАРТ ДЛЯ ГОСЗАКУПКИ:',
      '  Тип: проводная USB, оптическая | Разрешение: не менее 1000 dpi | Кнопок: не менее 3 (вкл. колесо) | Кабель: не менее 1.5 м | Гарантия: не менее 12 месяцев',
      '  НЕ включать без явного запроса: Bluetooth, беспроводной режим RF.',
      '',
      '- Тип (оптическая / лазерная)',
      '- Интерфейс подключения (USB — для проводной; RF 2.4 ГГц — для беспроводной)',
      '- Разрешение сенсора (не менее, dpi)',
      '- Количество кнопок (не менее, шт.)',
      '- Колесо прокрутки (наличие)',
      '- Длина кабеля (не менее, м) — для проводной',
      '- Совместимость с ОС (Astra Linux, Windows или эквивалентными)',
      '- Масса (не более, г)',
      '- Гарантия производителя (не менее, мес)',
...
// Хинт для внешнего USB оптического привода (CD/DVD)
const opticalDriveHint = [
  '⚠️ ОФИСНЫЙ СТАНДАРТ для госзакупки: если не указано "внутренний", указывать ВНЕШНИЙ USB-привод.',
  '',
  '- Исполнение (внешний USB / внутренний SATA)',
  '- Интерфейс подключения (USB 3.0 / USB 2.0)',
  '- Поддерживаемые форматы записи (DVD-R, DVD+R, DVD-RW, DVD+RW, CD-R, CD-RW)',
  '- Поддерживаемые форматы чтения (DVD-ROM, DVD-R, DVD+R, DVD-RW, DVD+RW, CD-ROM, CD-R, CD-RW)',
  '- Скорость записи DVD-R (не менее, x)',
  '- Скорость записи CD-R (не менее, x)',
  '- Скорость чтения DVD (не менее, x)',
  '- Скорость чтения CD (не менее, x)',
  '- Буферная память (не менее, МБ)',
  '- Уровень шума при работе (не более, дБ)',
  '- Потребляемая мощность (не более, Вт)',
  '- Питание (от USB без дополнительного адаптера — для внешних)',
  '- Габариты (не более, ШxГxВ мм)',
  '- Масса (не более, кг)',
  '- Гарантия производителя (не менее, мес)',
].join('\n');
...
function buildPrompt(
  ...
  const isWork = !!(g as any).isWork || isUniversalWorkType(row.type);
  ...
  const systemPrompt = `# ТЗ-ГЕНЕРАТОР v3.1
Ты — юрист‑закупщик и технический эксперт по государственным и корпоративным закупкам в России. Твоя задача — подготовить техническое задание (ТЗ) для закупки товара по законодательству РФ: при закупках по 44‑ФЗ соблюдать требования 44‑ФЗ, при закупках по 223‑ФЗ — требования 223‑ФЗ и внутренних документов заказчика. Документ должен быть готов к размещению в ЕИС без правок.

## АБСОЛЮТНЫЕ ЗАПРЕТЫ (никогда не нарушать)
1. НЕ вставлять в тело документа: [!], (!), [Требуется...], [уточнить], TODO, FIXME, **Примечание:**, "Значение уточняется", ЗАПОЛНИТЬ, ВСТАВИТЬ, N/A, TBD.
2. НЕ использовать неизмеримые формулировки: "современный", "высокопроизводительный", "по требованиям заказчика", "на усмотрение поставщика". НУЖНО: "не менее 16 ГБ", "не более 65 Вт".
3. НЕ упоминать бренды без "или эквивалент". Исключения (не требуют "или эквивалент"): стандарты (USB, HDMI, DDR4, DDR5, NVMe, Wi-Fi, Bluetooth, PCIe, RJ-45, IPS, VA, TN, OLED, ГОСТ, ТР ТС, ISO); отечественные ОС (Astra Linux, ALT Linux, РЕД ОС, Альт, Rosa Linux). "или выше" тоже является допустимым эквивалентом (напр. "Windows 10 или выше").
4. НЕ указывать устаревшие нормативы: ПП №878, №616, №925 (отменены с 23.12.2024). Для ТОВАРОВ: ПП №1875 + ПП №719. Для ПО: ПП №1236. Для УСЛУГ: профстандарты, ГОСТ Р (ПП №1875, №719, №1236 к услугам не применяются).
5. НЕ применять нормативы одной категории к другой: ПП №1875/№719 — только ТОВАРЫ/РАБОТЫ; ПП №1236 — только ПО; к УСЛУГАМ — только отраслевые ГОСТы и профстандарты.

## ДОПУСТИМЫЕ ФОРМУЛИРОВКИ В ХАРАКТЕРИСТИКАХ:
- "не менее X [единица]" | "не более X [единица]" | "от X до Y [единица]"
- "наличие" | "отсутствие" | "обязательно" | "поддерживается"
- "или эквивалент" | "или аналог" | "или выше" | "или более поздней версии"
- "не ниже класса «А»" | "класс A" — для энергоэффективности
...
• "Процессор Intel Core i5-12500"                              → "не менее 6 ядер, частота не менее 3,0 ГГц, кеш L3 не менее 18 МБ, архитектура x86-64"
• "2x DDR5 up to 5600MHz SO-DIMMs, up to 64GB"                → "Тип памяти: DDR5 или выше; объём не менее 8 ГБ; частота не менее 2400 МГц; слотов не менее 2 шт."
• "SSD 512 GB NVMe"                                           → "Накопитель SSD, объём не менее 512 ГБ, интерфейс NVMe M.2 или SATA"
• "Fan Cooler"                                                → "Система охлаждения: активная (вентилятор)" — или просто не указывать
• "120W PSU"                                                  → "Блок питания: мощность не менее 120 Вт"
• "204 x 208 x 54.8 mm (8.03 x 8.19 x 2.16 inch)"           → либо "Габариты не более 230 × 230 × 60 мм" (с запасом), либо не указывать если не важно
• "1.52 kg (3.35 lbs)"                                        → "Масса не более 2 кг"
• "Цвет: серебристый"                                         → не включать (не функциональный параметр)
• "Windows 11 Pro"                                            → "Совместимость с Astra Linux, РЕД ОС, ALT Linux или эквивалентной ОС из реестра Минцифры"
• "Гарантия производителя 1 год" для ПК/монитора              → "Гарантийный срок: не менее 36 месяцев"
• "Гарантия производителя 1 год" для клавиатуры/мыши          → "Гарантийный срок: не менее 12 месяцев"
• "Intel NIC 1GbE"                                            → "Сетевой интерфейс: Ethernet RJ-45, скорость не менее 1 Гбит/с"
• "Wi-Fi 6 (802.11ax), Bluetooth 5.3" в составе системного блока → НЕ включать (требования ИБ в госорганах); если беспроводной не нужен — просто не указывать
• "VESA: 100x100 mm"                                          → "Крепление VESA 75×75 мм и/или 100×100 мм — наличие"
• "ТОРП: Да"                                                  → НЕ включать в спецификацию товара
• "USB-токен Рутокен ЭЦП 3.0"                                 → "USB-токен криптографический или эквивалент: поддержка ГОСТ Р 34.10-2012, память не менее 64 КБ, интерфейс USB Type-A"
...
• НИКАКИХ МЕТА-ИНСТРУКЦИЙ в значениях характеристик — запрещено писать в поле "value": «Удалить эту строку», «Убрать данную позицию», «Уже указано выше», «Дублирование устранено». Если строку нужно убрать — просто НЕ включай её в JSON-ответ.
• Никаких фраз «ручной ввод», «своя услуга», «свой товар», «ручной импорт» — только корректные наименования объектов закупки.
• Никаких незакрытых плейсхолдеров [ВСТАВИТЬ], [УКАЗАТЬ], [НАИМЕНОВАНИЕ], [ФИО], [указать минимальную частоту], [указать конкретный срок] — если конкретных данных нет, используй отраслевые дефолты (например, «не менее 2,4 ГГц» для частоты ЦП), но НИКОГДА не оставляй плейсхолдер в документе.
• ВСЕ ЗНАЧЕНИЯ ХАРАКТЕРИСТИК ТОЛЬКО НА РУССКОМ ЯЗЫКЕ. Технические аббревиатуры (USB, HDMI, DDR5, NVMe, IPS) можно оставлять латиницей как имена стандартов, но описательный текст — только по-русски. Английские фразы типа «Fan Cooler», «2x DDR5 up to 5600MHz SO-DIMMs, up to 64GB», «204 x 208 x 54.8 mm (8.03 x 8.19 x 2.16 inch)» — ЗАПРЕЩЕНЫ.
• ЗАПРЕЩЕНО копировать значения из даташита производителя дословно на английском языке. Всегда переводи и переформулируй как требования (не менее / не более + метрические единицы).
• ЗАПРЕЩЕНЫ точные размеры в дюймах (inch) и вес в фунтах (lbs) — только метрические единицы (мм, кг).
...
• КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ РАСПЛЫВЧАТЫЕ ЗНАЧЕНИЯ — нарушение 44-ФЗ ст. 33:
  ...
  – «по усмотрению Заказчика» / «на усмотрение поставщика» / «по спецификации производителя» → НЕЛЬЗЯ
  – «согласно технической документации» / «согласно документации» → НЕЛЬЗЯ: дать конкретный параметр
  – «уточняется при поставке» / «определяется производителем» → НЕЛЬЗЯ
  – «не хуже аналогов» / «при необходимости» / «по модели поставки» → НЕЛЬЗЯ
  – «ТОРП: Да» / «ТОРП: Нет» — НИКОГДА не включать в спецификацию товара, это атрибут закупочной документации
...
• Wi-Fi и Bluetooth в составе ПК, системного блока или сервера — НЕ включать, если заказчик явно не запросил. Госорганы как правило запрещают беспроводные интерфейсы по требованиям ИБ. Если из даташита импортированы строки с Wi-Fi/BT и в запросе пользователя нет явного упоминания беспроводного подключения — исключить эти строки.
• Строка «ТОРП» / «Требования к товарам российского происхождения» — НЕ включать в таблицу характеристик; это поле заполняется в извещении о закупке, не в ТЗ.
• Объём корпуса в литрах / пинтах — не указывать; VESA-крепление с дюймами (100 x 100 inch) — ЗАПРЕЩЕНО, только мм.
• Точные модельные размеры типа «204 x 208 x 54.8 mm (8.03 x 8.19 x 2.16 inch)» или «1.52 kg (3.35 lbs)» — из даташита конкретной модели. Либо убрать, либо переформулировать как «не более X мм» / «не более Y кг».

ПРАВИЛА ГАРАНТИИ:
• Вычислительная техника (ПК, системные блоки, серверы, ноутбуки, мониторы, МФУ, принтеры): гарантия НЕ МЕНЕЕ 36 МЕСЯЦЕВ.
• Периферия (клавиатуры, мыши, веб-камеры, гарнитуры, хабы): гарантия не менее 12 месяцев.
• Сетевое и серверное оборудование: гарантия не менее 36 месяцев.
• Расходные материалы, кабели, мелкая переферия: гарантия не менее 12 месяцев.
• ПО и лицензии: указывать срок технической поддержки, не срок гарантии.
...
- МИНИМУМ характеристик: для аппаратного ИТ — не менее 25, для ПО — не менее 18, для услуг и работ — не менее 15
...
МАТРИЦА НОРМАТИВКИ ПО КАТЕГОРИЯМ ЗАКУПКИ (строго соблюдать):
Документ               | ТОВАРЫ | РАБОТЫ | УСЛУГИ | ПО/Лицензии
44-ФЗ от 05.04.2013    |   ДА   |   ДА   |   ДА   |    ДА
ПП РФ №1875 от 23.12.2024 |  ДА  |  ДА*  |   НЕТ  |   НЕТ
ПП РФ №719 от 17.07.2015  |  ДА  |  ДА*  |   НЕТ  |   НЕТ
ПП РФ №1236 от 16.11.2015 | НЕТ  |  НЕТ  |   НЕТ  |    ДА
ТР ТС 004/2011         |   ДА   |   ДА   |   НЕТ  |   НЕТ
ТР ТС 020/2011         |   ДА   |   ДА   |   НЕТ  |   НЕТ
Решение ЕЭК №105       |   ДА   |   НЕТ  |   НЕТ  |   НЕТ
ПП №878, №616, №925    |  УСТАРЕЛИ — НИКОГДА НЕ ССЫЛАТЬСЯ ни для одной категории
(*для РАБОТ: ПП №1875 применяется к материалам и оборудованию, используемым при выполнении работ)

КАТЕГОРИИ ЗАКУПКИ — определение и нормативная специфика:
ТОВАРЫ: физические предметы (техника, мебель, расходники, запчасти). ПП №1875 + ПП №719 обязательны.
РАБОТЫ: строительство, монтаж, прокладка, установка, настройка оборудования. ПП №1875 применяется к материалам, СНиП/СП — если строительная работа.
УСЛУГИ: обслуживание, поддержка, охрана, уборка, обучение, аудит. ПП №1875 и ПП №719 — НЕ ПРИМЕНЯЮТСЯ.
ПО/ЛИЦЕНЗИИ: программное обеспечение, лицензии, доступ к системам. Только ПП №1236 (реестр Минцифры).

СТРУКТУРА ТЗ ДЛЯ УСЛУГ (6 разделов):
Раздел 1 — Предмет договора: наименование, цель, место, период оказания.
Раздел 2 — Требования к услуге: перечень/состав услуг, периодичность, SLA, требования к персоналу, отчётность.
Раздел 3 — Требования к результату: измеримые, проверяемые показатели.
Раздел 4 — Порядок приёмки: документы, сроки, форма акта.
Раздел 5 — Требования к квалификации: лицензии (если лицензируемый вид), опыт, документы.
Раздел 6 — Нормативные акты: 44-ФЗ и специальные законы (ФЗ №273 для образования, ФЗ №99 для лицензируемых видов).
НЕ ВКЛЮЧАТЬ для услуг: ПП №1875, ПП №719, разделы про тару, упаковку, гарантию на товар.
SLA для услуг ТО: время реакции (не более N ч), устранение критической неисправности (не более N ч), доступность не менее 99%/мес.
SLA для ИТ-поддержки: Критичный (реакция ≤1ч/решение ≤4ч), Высокий (≤2ч/≤8ч), Средний (≤4ч/≤24ч), Низкий (≤8ч/≤72ч).

СТРУКТУРА ТЗ ДЛЯ РАБОТ (6 разделов):
Раздел 1 — Предмет контракта: наименование, объём, место выполнения.
Раздел 2 — Требования к выполнению работ: технология, последовательность, стандарты (ГОСТ, СНиП/СП).
Раздел 3 — Требования к материалам и оборудованию: марки не указывать, ссылки на категорию/класс + «или эквивалент». ПП №1875 к используемым материалам.
Раздел 4 — Требования к качеству и приёмке: форма акта (КС-2 если строительная), протоколы тестирования, схема as-built.
Раздел 5 — Требования к безопасности и допускам: СРО (если требуется по объёму), лицензии ФСТЭК/ФСБ если ИБ.
Раздел 6 — Нормативные акты: 44-ФЗ, ПП №1875, ГОСТ Р 53245-2008 (для СКС), иные применимые ГОСТы/СП.

СТРУКТУРА ТЗ ДЛЯ ПРОГРАММНОГО ОБЕСПЕЧЕНИЯ И ЛИЦЕНЗИЙ (6 разделов):
Раздел 1 — Предмет закупки: наименование ПО, тип (системное/прикладное/средство защиты), количество лицензий.
Раздел 2 — Требования к ПО: функциональные характеристики, поддерживаемые ОС (Astra Linux, ALT Linux, РЕД ОС или эквивалентные), совместимость форматов.
Раздел 3 — Требования к лицензии: тип (бессрочная/подписка), срок действия, количество рабочих мест/серверов, условия активации (офлайн/онлайн).
Раздел 4 — Требования к поставке и активации: носитель (электронная/физическая), документ подтверждения лицензии, срок поставки.
Раздел 5 — Техническая поддержка: режим (5x8/7x24), каналы обращений, SLA, обновления включены/нет, срок поддержки.
Раздел 6 — Нормативные акты: 44-ФЗ, ПП РФ №1236 от 16.11.2015 (реестр российского ПО). ПП №1875 и ПП №719 — НЕ ПРИМЕНЯЮТСЯ к ПО.
ОБЯЗАТЕЛЬНО для ПО: Включение в Единый реестр российских программ Минцифры (ч.5 ст.14 149-ФЗ). Если ПО включает УКЭП — 63-ФЗ.`;

  // ── Промпт для РАБОТ (монтаж, прокладка, установка) ──
  if (isWork) {
    const law175Example = getResolvedLaw175Meta(row.type, { nac_regime: 'none' });
    const isUniversalWork = isUniversalWorkType(row.type);
    const workName = isUniversalWork ? row.model : goodsName;
    const knownOkpd2Block = !isUniversalWork && okpd2 && okpd2 !== '00.00.00.000'
      ? `Код ОКПД2 уже определён системой: ${okpd2} — ${goodsName}. Используй этот код в meta.okpd2_code.`
      : '';
    return {
      system: systemPrompt,
      user: `Пользователь хочет заказать выполнение работ.
Вид работ: "${workName}"
${row.model && !isUniversalWork ? `Дополнительное описание / требования: "${row.model}"` : ''}
Объём / количество: ${row.qty}
${explicitCommercialTermsBlock ? `Коммерческие параметры из заявки:\n${explicitCommercialTermsBlock}\n` : ''}
${knownOkpd2Block}
${importedBlock}
${organizationBlock}

КАТЕГОРИЯ ЗАКУПКИ: РАБОТЫ (монтаж, прокладка, установка, настройка)
Нормативная база для работ: 44-ФЗ, ПП №1875 (применяется к материалам и оборудованию), ПП №719 (к материалам), СНиП/СП/ГОСТ если строительная работа.

ТВОЯ ЗАДАЧА:
1. ${isUniversalWork ? 'Определить правильный код ОКПД2 для данного вида работ.' : 'Подтвердить / уточнить код ОКПД2.'}
2. Полное наименование по ОКПД2.
3. Код КТРУ (если применимо, иначе пустая строка).
4. Национальный режим: для самих работ ставь "none"; укажи в law175_basis что ПП №1875 применяется к поставляемым материалам и оборудованию.
5. НЕ МЕНЕЕ 18 требований, охватывающих ВСЕ разделы ТЗ для работ:
   ...
   РАЗДЕЛ 2 — Требования к выполнению работ:
   - Технология и последовательность работ
   - Применимые стандарты (ГОСТ, СНиП/СП) — указать конкретные
   - Требования к маркировке, тестированию, документированию

   РАЗДЕЛ 3 — Требования к материалам и оборудованию:
   - Категория/класс материалов (без конкретных брендов, с «или эквивалент»)
   - Применение ПП №1875 к используемым материалам и оборудованию

   РАЗДЕЛ 4 — Требования к качеству и приёмке:
   - Форма акта приёмки (КС-2 если строительная работа, иначе акт выполненных работ)
   - Протоколы тестирования / испытаний (если применимо)
   - Исполнительная документация (схема as-built)

   РАЗДЕЛ 5 — Требования к допускам и квалификации:
   - СРО (указать обязательность / необязательность для данного вида работ)
   - Лицензии ФСТЭК/ФСБ (если работы связаны с защитой информации)
   - Сертификаты производителя оборудования (если применимо)

   РАЗДЕЛ 6 — Нормативные акты:
   - 44-ФЗ, ПП №1875, применимые ГОСТ, СНиП/СП

Правила для meta:
- law175_status: "none" (для самих работ как объекта закупки).
- law175_basis: объясни что ПП №1875 применяется к материалам и оборудованию, используемым при выполнении работ, но не к самому факту выполнения работ.
- nac_regime: "none".

Ответ СТРОГО в JSON (без markdown, без пояснений):
{
  "meta": {
    "okpd2_code": "${!isUniversalWork && okpd2 && okpd2 !== '00.00.00.000' ? okpd2 : 'XX.XX.XX.XXX'}",
    "okpd2_name": "${!isUniversalWork ? goodsName : 'Полное наименование работы по ОКПД2'}",
    "ktru_code": "",
    "nac_regime": "none",
    "law175_status": "${law175Example.status}",
    "law175_basis": "${law175Example.promptBasis}"
  ...
  "specs": [
    {"group":"Предмет контракта","name":"Состав работ","value":"конкретный перечень работ","unit":"—"},
    {"group":"Требования к материалам","name":"Нацрежим на материалы","value":"материалы и оборудование должны соответствовать ПП РФ №1875 от 23.12.2024","unit":"—"},
    {"group":"Приёмка","name":"Форма акта","value":"акт выполненных работ или форма КС-2 (при строительстве)","unit":"—"}
  ]
}`
    };
  }

  // ── Промпт для УСЛУГ (ТО, поддержка, обучение, охрана, уборка и т.д.) ──
  if (isService) {
    const law175Example = getResolvedLaw175Meta(row.type, { nac_regime: 'none' });
    const isUniversalSvc = isUniversalServiceType(row.type);
    const serviceName = isUniversalSvc ? row.model : goodsName;
    const knownOkpd2BlockSvc = !isUniversalSvc && okpd2 && okpd2 !== '00.00.00.000'
      ? `Код ОКПД2 уже определён системой: ${okpd2} — ${goodsName}. Используй этот код в meta.okpd2_code.`
      : '';
    return {
      system: systemPrompt,
      user: `Пользователь хочет закупить услугу.
Наименование услуги: "${serviceName}"
${row.model && !isUniversalSvc ? `Дополнительное описание / требования: "${row.model}"` : ''}
Количество / объём: ${row.qty}
${explicitCommercialTermsBlock ? `Коммерческие параметры из заявки:\n${explicitCommercialTermsBlock}\n` : ''}
${knownOkpd2BlockSvc}
${importedBlock}
${organizationBlock}

КАТЕГОРИЯ ЗАКУПКИ: УСЛУГИ
Нормативная база: 44-ФЗ. ПП №1875 и ПП №719 — НЕ ПРИМЕНЯЮТСЯ к услугам. ПП №1236 — только для ПО.

ТВОЯ ЗАДАЧА:
1. ${isUniversalSvc ? 'Определить правильный код ОКПД2 услуги.' : 'Подтвердить / уточнить код ОКПД2.'}
2. Полное наименование по ОКПД2.
3. Код КТРУ (если применимо, иначе пустая строка).
4. Национальный режим: для услуг "none".
5. НЕ МЕНЕЕ 18 требований, охватывающих ВСЕ разделы ТЗ для услуг:

   РАЗДЕЛ 1 — Предмет договора:
   - Наименование и цель услуги
   - Место оказания услуги
   - Период оказания (дата начала / окончания, или количество месяцев)

   РАЗДЕЛ 2 — Требования к услуге:
   - Состав и перечень услуг (конкретный перечень операций/функций)
   - Периодичность / режим оказания (дни, часы, количество выездов)
   - SLA — уровень сервиса: время реакции на заявку, время устранения, доступность
   - Требования к персоналу Исполнителя (квалификация, количество)
   - Требования к отчётности (форма, периодичность)

   РАЗДЕЛ 3 — Требования к результату:
   - Измеримые, проверяемые показатели результата услуги

   РАЗДЕЛ 4 — Порядок приёмки:
   - Форма подтверждающих документов (акт, отчёт, журнал)
   - Сроки подписания акта

   РАЗДЕЛ 5 — Требования к квалификации и лицензиям:
   - Если деятельность лицензируется (образование, медицина, охрана и т.д.) — требование лицензии со ссылкой на закон
   - Иные квалификационные требования (допуски, сертификаты)

   РАЗДЕЛ 6 — Нормативные акты:
   - 44-ФЗ и применимые специальные законы

Правила для meta:
- nac_regime: "none" (ПП №1875 к услугам не применяется).
- law175_status: "none".
- law175_basis: укажи что ПП РФ №1875 не применяется к данной категории закупки (услуги), согласно матрице нормативных актов.

ЗАПРЕЩЕНО включать в ТЗ на услугу: ПП №1875, ПП №719, раздел о таре и упаковке, гарантию на товар.

Ответ СТРОГО в JSON (без markdown, без пояснений):
{
  "meta": {
    "okpd2_code": "${!isUniversalSvc && okpd2 && okpd2 !== '00.00.00.000' ? okpd2 : 'XX.XX.XX.XXX'}",
    "okpd2_name": "${!isUniversalSvc ? goodsName : 'Полное название услуги по ОКПД2'}",
    "ktru_code": "",
    "nac_regime": "none",
    "law175_status": "${law175Example.status}",
    "law175_basis": "${law175Example.promptBasis}"
  ...
  "specs": [
    {"group":"Предмет договора","name":"Наименование услуги","value":"конкретное наименование услуги","unit":"—"},
    {"group":"Состав услуг","name":"Перечень услуг","value":"конкретный перечень операций и функций","unit":"—"},
    {"group":"Сроки и SLA","name":"Срок оказания услуг","value":"не более 30","unit":"календарных дней"},
    {"group":"Сроки и SLA","name":"Время реакции на заявку","value":"не более 4","unit":"рабочих часов"},
    {"group":"Приёмка","name":"Подтверждающий документ","value":"ежемесячный акт оказанных услуг и отчёт","unit":"—"}
  ]
}`
    };
  }
...
${isSW ? `Национальный режим — ПО (только ПП РФ № 1236 от 16.11.2015):
- ПП РФ № 1875 к программному обеспечению НЕ ПРИМЕНЯЕТСЯ
- ПО ОБЯЗАТЕЛЬНО в реестре Минцифры России (ч.5 ст.14 149-ФЗ, ПП №1236)
...
Ответ СТРОГО в JSON без пояснений:
{
  ...
  "specs": [
    {"group":"Дополнительные характеристики","name":"Наименование новой характеристики","value":"конкретное значение","unit":"тип"}
      ...
      const processedExtra = postProcessSpecs(extraSpecs, row.type);
  ...
  const fetchInternetCandidateForRow = useCallback(async (row: GoodsRow): Promise<SpecsCandidate | null> => {
    ...
    const processed = adjustSpecsForCommercialContext(row, postProcessSpecs(specs, row.type));
  ...
  const mutation = useMutation({
    mutationFn: async (options?: GenerateOptions) => {
          ...
          if (batchAbleIs.length >= 2) {
            ...
            const generateSingleRowFallback = async (ri: number): Promise<void> => {
              ...
              try {
                ...
                const { meta, specs } = parseAiResponse(raw);
                const processed = postProcessSpecs(specs, currentRow.type);
            ...
            try {
              ...
              await Promise.allSettled(batchAbleIs.map(async (ri, j) => {
                ...
                if (item && item.specs && item.specs.length > 0) {
                  const processed = postProcessSpecs(item.specs, currentRow.type);
                  const adjustedSpecs = adjustSpecsForCommercialContext(currentRow, processed);
      ...
      try {
        ...
        // ── ИИ-верификация ЕИС-готовности (фоновый второй проход) ──────────────
        if (doneRows.length > 0) {
          const runVerification = async (currentRows: GoodsRow[]) => {
            const verifiableRows = currentRows.filter(
              r => r.status === 'done' && r.specs && r.specs.length > 0
            );
            if (verifiableRows.length === 0) return;

            setRows(prev => prev.map(r =>
              r.status === 'done' && r.specs && r.specs.length > 0
                ? { ...r, verifying: true }
                : r
            ));

            const VERIFY_CONCURRENCY = 2;
            for (let k = 0; k < verifiableRows.length; k += VERIFY_CONCURRENCY) {
              const chunk = verifiableRows.slice(k, k + VERIFY_CONCURRENCY);
              await Promise.allSettled(chunk.map(async (r) => {
                try {
                  const result = await verifySpecsWithAI(
                    r.specs || [],
                    r.type,
                    provider,
                    model,
                  );
                  const hasAnyFixes = result.fixedSpecs.some(s => s._fixed);
                  setRows(prev => prev.map(row =>
                    row.id === r.id
                      ? {
                          ...row,
                          verifying: false,
                          verification: result,
                          specs: hasAnyFixes ? result.fixedSpecs : row.specs,
                        }
                      : row
                  ));
                } catch {
                  setRows(prev => prev.map(row =>
                    row.id === r.id ? { ...row, verifying: false } : row
                  ));
                }
              }));
            }
    ...
    try {
      const validateRows = rows.map((row) => {
        ...
        return {
          ...
          qty_unit: 'шт.',
          category: cat,
          okpd2_code: (row as { okpd2?: string }).okpd2 || '',
            ...
            <div className="workspace-auth-collapse-inner">
              {backendUser ? (
                <div style={{ background: '#0F3B1E', border: '1px solid #166534', borderRadius: 8, padding: '12px 16px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 12, fontSize: 13 }}>
                  ...
                  {(backendUser!.role === 'pro' || backendUser!.role === 'admin') && backendUser!.subscription_expires_at && (
                    <span style={{ fontSize: 11, color: '#86EFAC', opacity: 0.85 }}>
                      Действует до: {new Date(backendUser!.subscription_expires_at).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                    </span>
                  )}
