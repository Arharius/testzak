...
- **Уровень 3 — Авто-исправление** (`POST /api/tz/auto-fix`):
  - TEST-01: удалить мета-комментарии из значений характеристик
  - TEST-02: удалить строки с запрещёнными формулировками 44-ФЗ  
  - TEST-03: добавить «или эквивалент» после брендов
  - TEST-04: LLM-фикс неизмеримых значений (макс 10 вызовов)
  - TEST-05: дедуплицировать характеристики (оставить более полный вариант)
  - TEST-07 (NEW): исправить qty=0→1, добавить единицы измерения «шт.»
  - TEST-08 (NEW): заменить устаревшие ПП №878/616/925 → №1875, добавить обязательные ссылки (44-ФЗ, 1875, 719), убрать нерелевантные для услуг
  - Возвращает `{rows, fix_report, validation, fixed_full_text}` — `fixed_full_text` сохраняется в Workspace state и используется в последующих валидациях
  - Фронтенд: цикл до 3 итераций (`runAutoFix`), `fixedFullText` state передаётся в оба endpoint-а
  - `FixReportPanel` в модале: зелёный блок с 11 типами действий (action labels)
