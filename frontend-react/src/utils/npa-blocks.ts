...
export function buildSection2Rows(
  ...
  const isSupport = goodsType === 'osSupport' || goodsType === 'supportCert';
  const lawRef44 = is223 ? '' : ' (ст. 14 Федерального закона от 05.04.2013 № 44-ФЗ)';

  // ── Обязательные блоки 2.1–2.4 для любого предмета закупки ──
  const rows: Row2[] = [
    ['2.1. Характеристики предмета закупки',
      'Технические и функциональные характеристики поставляемого товара указаны в Приложениях к настоящему Техническому заданию'],
    ['2.2. Функциональное состояние',
      'Товар должен быть полнофункциональным, не демонстрационным'],
    ['2.3. Качество исполнения',
      'Товар должен быть без дефектов конструкции, материалов и функционирования'],
    ['2.4. Соответствие обязательным требованиям',
      'Товар должен соответствовать требованиям качества и безопасности, установленным законодательством Российской Федерации'],
  ];

  // ── Блоки 2.5–2.9 в зависимости от типа предмета закупки ──
  if (isSupport) {
    rows.push(
      ['2.5. Предмет поставки',
        'Поставляется сертификат технической поддержки (продление подписки правообладателя) в объёме, достаточном для обслуживания закупаемого программного обеспечения'],
      ['2.6. Правовое основание',
        'Право на техническую поддержку передаётся на основании документов правообладателя'],
      ['2.7. Документация',
        'Поставщик передаёт регламент технической поддержки, описание SLA, порядок регистрации обращений и иные документы правообладателя на русском языке'],
      ['2.8. Активация',
        'Поставщик обеспечивает регистрацию сертификата технической поддержки в личном кабинете правообладателя и передачу Заказчику подтверждающих документов'],
      ['2.9. Национальный режим',
        'Национальный режим (ПП РФ № 1875): к данному виду услуг не применяется'],
    );
  } else if (isSW) {
    // Программное обеспечение: пп. 2.5/2.9 заменяются специальными блоками
    rows.push(
      ['2.5. Включение в реестр российского ПО',
        'Программное обеспечение должно быть включено в Единый реестр российских программ для электронных вычислительных машин и баз данных (реестр Минцифры России) в соответствии с ПП РФ от 16.11.2015 № 1236. Участник закупки обязан указать номер реестровой записи в составе заявки'],
      ['2.6. Правовое основание поставки',
        'Право использования программного обеспечения передаётся на основании лицензионного договора (сублицензионного соглашения) в соответствии с частью IV Гражданского кодекса Российской Федерации'],
      ['2.7. Лицензионная чистота',
        'Программное обеспечение является лицензионно чистым. Поставщик гарантирует правомерность использования'],
      ['2.8. Документация',
        'Документация поставляется на русском языке, включая инструкции производителя (правообладателя)'],
      ['2.9. Форма поставки',
        'Поставка осуществляется в электронной форме: дистрибутивы, ключи активации, лицензионные документы, сертификаты технической поддержки'],
    );
  } else {
    // Товар (оборудование, промышленные товары)
    rows.push(
      ['2.5. Состояние товара',
        'Товар заводской сборки, серийный, новый, не бывший в употреблении'],
      ['2.6. Правовая чистота',
        'Товар не является предметом залога, не находится под арестом, не обременён правами третьих лиц'],
      ['2.7. Физическое состояние',
        'Товар без следов механических повреждений'],
      ['2.8. Документация',
        'Документация поставляется на русском языке, включая инструкции производителя'],
    );

    // 2.9 — национальный режим / подтверждение происхождения
    let text29: string;
    if (nacRegime === 'pp878' || nacRegime === 'pp616') {
      text29 = `Товар должен иметь подтверждение производства на территории государств — членов ЕАЭС в соответствии с ПП РФ от 23.12.2024 № 1875${lawRef44} и ПП РФ от 17.07.2015 № 719 (при применимости). Реестровая запись в реестре российской промышленной продукции или евразийском реестре промышленных товаров${basisSuffix}`;
    } else if (nacRegime === 'pp1236') {
      // ПО-режим на HW-типе — обрабатываем как промышленный товар
      text29 = `Товар должен иметь подтверждение производства на территории государств — членов ЕАЭС в соответствии с ПП РФ от 23.12.2024 № 1875${lawRef44} и ПП РФ от 17.07.2015 № 719 (при применимости)${basisSuffix}`;
    } else {
      text29 = `Национальный режим (ПП РФ № 1875): к данному виду товаров не применяется${basisSuffix}`;
    }
    rows.push(['2.9. Национальный режим и подтверждение происхождения товара', text29]);
  }

  // ── 2.10 — твёрдая цена (обязательно для всех) ──
  const contractWord = is223 ? 'договора' : 'контракта';
  rows.push(['2.10. Твёрдая цена',
    `Цена ${contractWord} является твёрдой на весь срок исполнения`]);

  // ── Специальные требования по типу товара ──
  const specialMap: Record<string, Row2[]> = {
    pc:         [['Совместимость с отечественными ОС', `Товар должен быть совместим с отечественными операционными системами, включёнными в Единый реестр российских программ для ЭВМ и БД Минцифры России, в том числе: Astra Linux Special Edition, ALT Linux, РЕД ОС или эквивалентными${osNote}`]],
    laptop:     [['Совместимость с отечественными ОС', `Товар должен быть совместим с отечественными операционными системами, включёнными в Единый реестр российских программ для ЭВМ и БД Минцифры России, в том числе: Astra Linux Special Edition, ALT Linux или эквивалентными${osNote}`]],
    server:     [['Совместимость с отечественными ОС', `Товар должен быть совместим с отечественными операционными системами серверной редакции, включёнными в Единый реестр российских программ для ЭВМ и БД Минцифры России, в том числе: Astra Linux Special Edition или эквивалентными; поддержка гипервизора KVM; поддержка IPMI/Redfish для удалённого управления${osNote}`]],
    monoblock:  [['Совместимость с отечественными ОС', `Товар должен быть совместим с отечественными операционными системами, включёнными в Единый реестр российских программ для ЭВМ и БД Минцифры России, в том числе: Astra Linux Special Edition, ALT Linux или эквивалентными${osNote}`]],
    firewall:   [
    ...
    crypto:     [
      ...
      ['Сертификат ФСТЭК', 'Наличие действующего сертификата ФСТЭК России по требованиям к уровню доверия не ниже 4-го уровня'],
    ],
    ...
    email:      [
      ['Совместимость с отечественным ПО', `Программное обеспечение должно функционировать на отечественных операционных системах (Astra Linux Special Edition, ALT Linux или эквивалентных), включённых в Единый реестр Минцифры России${osNote}`],
      ['Интеграция со службой каталогов', 'Почтовая система должна поддерживать интеграцию с ALD Pro / LDAP-каталогом либо эквивалентной службой каталогов для централизованного управления пользователями'],
      ['Высокая доступность', 'Почтовая система должна поддерживать кластеризацию и иные механизмы высокой доступности для обеспечения непрерывности сервиса'],
      ['Миграция', 'Почтовая система должна обеспечивать миграцию с Microsoft Exchange с сохранением почтовых сообщений, календарей и адресных книг'],
    ],
    vks:        [
    ...
    vdi:        [
      ...
      ['Совместимость с платформой виртуализации', 'Платформа VDI должна поддерживать работу с гипервизором и инфраструктурой ПК «Брест» либо эквивалентной отечественной платформой виртуализации'],
      ['Клиентский доступ', 'Система должна обеспечивать доступ к рабочим местам через веб-браузер (HTML5) и/или тонкие клиенты без ухудшения функциональности'],
      ['Мультиарендность и лицензирование', 'Платформа должна поддерживать мультиарендность, а лицензионная схема должна предусматривать конкурентных пользователей (CCU) и/или именованных пользователей'],
      ['Шлюз подключений', 'Система должна обеспечивать работу через шлюз подключений без обязательного использования VPN, если такой режим предусмотрен правообладателем'],
    ],
    edr:        [
    ...
    backup_sw:  [
      ['Совместимость с отечественным ПО', 'Программное обеспечение должно функционировать на отечественных ОС (Astra Linux, ALT Linux или эквивалентных) и поддерживать резервное копирование отечественных СУБД (PostgreSQL и др.)'],
      ['Совместимость с виртуализацией', 'Система должна поддерживать резервное копирование виртуальных машин платформы ПК «Брест» без установки агентов (agentless), если такой режим предусмотрен правообладателем'],
      ['Состав поставки', 'Поставка должна предусматривать серверную часть и требуемое количество агентов (клиентов) для резервного копирования БД и прикладных систем'],
      ['Ролевая модель и шифрование', 'Система должна поддерживать ролевую модель доступа (RBAC) и шифрование резервных копий с использованием отечественных криптографических алгоритмов ГОСТ либо сертифицированных СКЗИ, если это предусмотрено правообладателем'],
      ['Дедупликация и лицензирование', 'Система должна обеспечивать глобальную дедупликацию на стороне клиента и сервера, а лицензионная схема должна предусматривать объём защищаемых данных (ТБ) и/или количество клиентов'],
    ],
    ...
    virt:       [
      ...
      ['Управление сетями и хранилищами', 'Платформа должна обеспечивать управление виртуальными сетями, кластерами и хранилищами на базе Ceph / LVM либо эквивалентных технологий'],
      ['Средства защиты информации', 'Платформа должна поддерживать встроенные средства защиты информации Astra Linux, включая мандатное управление доступом к виртуальным машинам, либо эквивалентные механизмы разграничения доступа'],
      ['Лицензирование', 'Лицензирование должно предусматривать вариант по количеству физических процессоров (сокетов) серверов виртуализации, если такая схема применяется правообладателем'],
    ],
    firewall_sw:[
    ...
    ldap:       [
      ...
      ['Миграция и централизованное управление', 'Служба каталогов должна обеспечивать централизованное управление пользователями и ПК, поддерживать групповые политики на базе SaltStack либо эквивалентного механизма, автоматизированную установку ОС по сети, интеграцию с DHCP/DNS и миграцию из Microsoft Active Directory с сохранением структуры'],
      ['Управление сайтами и репликацией', 'Система должна обеспечивать управление иерархией подразделений, сайтами и топологией репликации каталога'],
      ['Лицензирование', 'Лицензионная схема должна предусматривать серверную лицензию (контроллер домена) и клиентские лицензии CAL на каждое устройство или пользователя либо лицензии на управляемые объекты'],
    ],
    osSupport:   [
      ['Предмет поддержки', 'Сертификат технической поддержки должен распространяться на операционную систему Astra Linux Special Edition, РЕД ОС либо эквивалентную отечественную ОС в требуемой редакции'],
      ['Обновления и безопасность', 'В период действия сертификата Заказчику должен предоставляться доступ к обновлениям, патчам безопасности и материалам базы знаний правообладателя'],
      ['SLA привилегированного уровня', 'Для привилегированного уровня технической поддержки должен быть обеспечен режим 24x7 и время реакции на критический инцидент не более 1 часа'],
      ['Мажорные обновления', 'Сертификат технической поддержки должен предоставлять право на получение новых мажорных версий операционной системы в период действия поддержки'],
    ],
    supportCert: [
      ['Уровень сервиса', 'Сертификат технической поддержки должен предусматривать выбранный уровень обслуживания (Стандартный / Привилегированный / эквивалентный) с фиксированными SLA'],
      ['Экосистема Astra', 'Сертификат поддержки может распространяться на программные продукты экосистемы Astra, включая ALD Pro, ПК «Брест», Termidesk, RuPost и RuBackup'],
      ['SLA привилегированного уровня', 'Для привилегированного уровня технической поддержки должен быть обеспечен режим 24x7 и время реакции на критический инцидент не более 1 часа'],
      ['Мажорные обновления', 'Сертификат технической поддержки должен предоставлять право на получение новых мажорных версий поддерживаемого программного продукта в период действия поддержки'],
    ],
    reporting:  [
  ...
  // Добавляем специальные требования по типу — нумерация продолжается от rows.length + 1
  const extraData = specialMap[goodsType] ?? [];
  extraData.forEach(([label, val]) => {
    rows.push([`2.${rows.length + 1}. ${label}`, val]);
  });
...
export function buildSection4Rows(goodsType: string, lawMode: LawMode): Row2[] {
  ...
  if (goodsType === 'osSupport' || goodsType === 'supportCert') {
    return [
      ['4.1. Срок активации', `Регистрация сертификата технической поддержки у правообладателя осуществляется не позднее 5 (пяти) рабочих дней с даты заключения ${contractWord}`],
      ['4.2. Период действия', 'Период действия сертификата технической поддержки определяется условиями закупки и должен составлять не менее 12 месяцев либо иной срок, прямо указанный в спецификации закупки'],
      ['4.3. Сервисные обязательства', 'Поставщик обеспечивает активацию выбранного уровня поддержки, передачу SLA и каналов обращения, а также контроль корректности регистрации Заказчика в системе правообладателя. Для привилегированного уровня поддержки должен обеспечиваться режим 24x7 и время реакции на критический инцидент не более 1 часа; для стандартного уровня должны быть доступны консультации по телефону и электронной почте'],
      ['4.4. Обновления и база знаний', 'В течение срока действия сертификата Заказчику предоставляется доступ к обновлениям, патчам безопасности, базе знаний и иным материалам, предусмотренным соответствующим уровнем поддержки, включая право на получение мажорных обновлений программного продукта без дополнительной оплаты, если это предусмотрено условиями правообладателя'],
      ['4.5. Подтверждающие документы', 'Поставщик передаёт сертификат / подтверждение активации, регистрационный номер поддержки, условия SLA, перечень доступных каналов обращения и иные документы правообладателя на русском языке'],
    ];
  }

  if (isSW) {
    const docPackage = is223
      ? 'Факт поставки подтверждается подписанием акта приёма-передачи лицензий/ключей активации по форме, установленной Заказчиком. Поставщик предоставляет копию лицензионного договора или сублицензионного соглашения с указанием номера реестровой записи в Едином реестре российских программ'
      : 'Факт поставки подтверждается подписанием акта приёма-передачи лицензий/ключей активации. Поставщик предоставляет копию лицензионного договора или сублицензионного соглашения с указанием номера реестровой записи в Едином реестре российских программ';
    return [
  ...
  const docPackage = is223
    ? `первичный учётный документ по форме, предусмотренной договором (УПД или акт приёма-передачи), паспорт (формуляр) изделия, эксплуатационная документация на русском языке, гарантийный талон производителя (при наличии), выписка из реестра российской радиоэлектронной продукции (при применении ПП РФ № 1875)`
    : `универсальный передаточный документ (УПД) или товарная накладная (ТОРГ-12), паспорт (формуляр) изделия, эксплуатационная документация на русском языке, гарантийный талон производителя (при наличии), выписка из реестра российской радиоэлектронной продукции (при применении ПП РФ № 1875)`;
...
export function buildSection5Rows(goodsType: string, lawMode: LawMode): Row2[] {
  ...
  const hwRows: Row2[] = [
    ...
    ['5.2. Год выпуска', 'Актуальная стабильная версия, официально поддерживаемая производителем на момент поставки (год выпуска подтверждается серийным номером или сопроводительными документами)'],
