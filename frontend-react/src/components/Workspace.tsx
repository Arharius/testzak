...
function buildPrompt(row: GoodsRow, lawMode: LawMode): string {
  ...
  const specHints: Record<string, string> = {
    pc:         '- Корпус (тип, цвет)\n- Процессор (тип, кол-во ядер, частота, кэш)\n- Оперативная память (тип, объём, частота)\n- Накопитель (тип SSD/HDD, объём)\n- Видеокарта (тип, видеопамять)\n- Интерфейсы (USB 2.0, USB 3.0, HDMI, DisplayPort, VGA, Ethernet, аудио)\n- Сетевые интерфейсы (Ethernet 1Гбит/с, Wi-Fi, Bluetooth)\n- Блок питания (мощность)\n- Предустановленная ОС',
    laptop:     '- Экран (диагональ, разрешение, тип матрицы, яркость)\n- Процессор (кол-во ядер, частота, кэш)\n- Оперативная память (тип, объём, максимальный объём)\n- Накопитель (тип SSD, объём)\n- Видеокарта (тип, видеопамять)\n- Аккумулятор (ёмкость Вт·ч, время работы в часах)\n- Интерфейсы (USB, HDMI, Wi-Fi стандарт, Bluetooth)\n- Камера, микрофон\n- Вес (кг), габариты\n- Предустановленная ОС',
    monoblock:  '- Экран (диагональ, разрешение, тип матрицы)\n- Процессор (кол-во ядер, частота)\n- Оперативная память (тип, объём)\n- Накопитель (тип SSD, объём)\n- Видеокарта\n- Веб-камера, микрофон, динамики\n- Интерфейсы (USB, HDMI, Ethernet, Wi-Fi)\n- Предустановленная ОС',
    tablet:     '- Экран (диагональ, разрешение, тип матрицы, сенсорный)\n- Процессор (кол-во ядер, частота)\n- Оперативная память (объём)\n- Встроенная память (объём)\n- Камера (основная, фронтальная)\n- Аккумулятор (ёмкость мА·ч)\n- Беспроводные интерфейсы (Wi-Fi, Bluetooth, LTE/5G)\n- ОС\n- Вес (г)',
    thinClient: '- Процессор (кол-во ядер, частота)\n- Оперативная память (тип, объём)\n- Встроенный накопитель (тип, объём)\n- Видеовыходы (HDMI, DisplayPort, кол-во мониторов)\n- Сетевые интерфейсы (Ethernet, Wi-Fi)\n- USB-порты (кол-во, тип)\n- Поддерживаемые протоколы (RDP, Citrix ICA, PCoIP)\n- ОС\n- Потребляемая мощность',
    server:     '- Форм-фактор (Tower/Rack 1U/2U/4U)\n- Процессор (кол-во сокетов, кол-во ядер, частота, кэш)\n- Оперативная память (тип, объём, кол-во слотов, макс. объём)\n- Накопители (тип HDD/SSD, форм-фактор, кол-во отсеков, макс. объём)\n- RAID-контроллер (уровни RAID)\n- Сетевые интерфейсы (кол-во портов, скорость)\n- Блок питания (мощность, резервирование)\n- Управление (IPMI/BMC/iLO/iDRAC)\n- Предустановленная ОС',
    monitor:    '- Диагональ (дюймы)\n- Разрешение (Full HD / QHD / 4K)\n- Тип матрицы (IPS/VA/TN)\n- Яркость (кд/м²)\n- Контрастность (статическая)\n- Время отклика (мс)\n- Угол обзора (горизонт./вертик.)\n- Частота обновления (Гц)\n- Интерфейсы (HDMI, DisplayPort, VGA, USB-C)\n- Регулировка высоты, наклона, поворота\n- VESA крепление\n- Потребляемая мощность',
    printer:    '- Тип печати (лазерный/светодиодный/струйный)\n- Цветность (монохромный/цветной)\n- Формат бумаги (A4/A3)\n- Скорость печати (стр/мин)\n- Разрешение печати (dpi)\n- Время выхода первой страницы (с)\n- Ресурс картриджа (стр)\n- Лоток подачи (ёмкость, листов)\n- Двусторонняя печать\n- Интерфейсы (USB, Ethernet, Wi-Fi)\n- Нагрузка (стр/мес)',
    mfu:        '- Функции (печать, копирование, сканирование, факс)\n- Тип печати (лазерный/светодиодный)\n- Цветность\n- Формат бумаги (A4/A3)\n- Скорость печати (стр/мин)\n- Разрешение печати/сканирования (dpi)\n- Устройство автоподачи документов (ADF, листов)\n- Двусторонняя печать\n- Лоток подачи (ёмкость, листов)\n- Интерфейсы (USB, Ethernet, Wi-Fi)\n- Нагрузка (стр/мес)',
    scanner:    '- Тип сканера (планшетный/потоковый/документ-сканер)\n- Формат (A4/A3)\n- Оптическое разрешение (dpi)\n- Скорость сканирования (стр/мин, двусторонний)\n- Устройство автоподачи (ADF/RADF, листов)\n- Интерфейсы (USB, Ethernet, Wi-Fi)\n- Поддерживаемые форматы (PDF, JPEG, TIFF)\n- Суточная нагрузка (стр/день)',
    switch:     '- Кол-во портов Ethernet (тип, скорость: 100 Мбит/с, 1 Гбит/с, 10 Гбит/с)\n- Кол-во uplink-портов SFP/SFP+\n- Управляемость (управляемый L2/L3, неуправляемый)\n- Поддержка PoE/PoE+ (бюджет мощности)\n- Коммутационная матрица (Гбит/с)\n- Таблица MAC-адресов (записей)\n- Протоколы (VLAN 802.1Q, STP/RSTP, LACP, SNMP, QoS)\n- Монтаж (rack 19\"/desktop)\n- Потребляемая мощность',
    router:     '- Кол-во WAN-портов (тип, скорость)\n- Кол-во LAN-портов (скорость)\n- Пропускная способность (Мбит/с)\n- Протоколы маршрутизации (OSPF, BGP, RIP, статическая)\n- NAT, DHCP, DNS\n- VPN (IPsec, L2TP, GRE, PPTP, OpenVPN)\n- Межсетевой экран (Stateful Firewall)\n- QoS\n- Процессор, оперативная память\n- Монтаж (rack/desktop)',
    firewall:   '- Пропускная способность межсетевого экрана (Гбит/с)\n- Пропускная способность IPS (Гбит/с)\n- Пропускная способность VPN (Гбит/с)\n- Кол-во портов Ethernet (тип, скорость)\n- Кол-во VPN-туннелей\n- Функциональность (IPS/IDS, DPI, URL-фильтрация, антивирус, антиспам)\n- Кол-во одновременных сессий\n- Управление (веб-интерфейс, CLI, централизованное)\n- Сертификат ФСТЭК',
    accessPoint:'- Стандарт Wi-Fi (Wi-Fi 5/6/6E)\n- Частотные диапазоны (2.4 ГГц, 5 ГГц, 6 ГГц)\n- Скорость (Мбит/с по диапазонам)\n- Кол-во одновременных клиентов\n- Антенны (тип, кол-во, коэффициент усиления)\n- Порты Ethernet (скорость)\n- Питание PoE (802.3af/at)\n- Управление (контроллерное/standalone/облачное)\n- Монтаж (потолок/стена)',
    ups:        '- Тип (линейно-интерактивный/онлайн)\n- Мощность (ВА / Вт)\n- Входное напряжение (диапазон, В)\n- Выходное напряжение (В)\n- Форма выходного сигнала (синусоида/ступенчатая)\n- Время автономной работы (мин, при нагрузке)\n- Кол-во выходных розеток (тип)\n- Интерфейсы управления (USB, Ethernet/SNMP)\n- Тип аккумулятора\n- Габариты, вес',
    projector:  '- Технология (DLP/LCD/LCoS)\n- Яркость (лм)\n- Разрешение (Full HD/4K)\n- Контрастность\n- Срок службы лампы (часов)\n- Интерфейсы (HDMI, VGA, USB)\n- Расстояние проекции\n- Вес (кг)',
    ...
    hdd:        '- Форм-фактор (3.5\"/2.5\")\n- Интерфейс (SATA III/SAS)\n- Объём (ТБ)\n- Скорость вращения (об/мин)\n- Объём кэша (МБ)\n- Назначение (серверный/NAS/десктопный)\n- MTBF (часов)',
    ram:        '- Тип (DDR4/DDR5)\n- Форм-фактор (DIMM/SO-DIMM/RDIMM/LRDIMM)\n- Объём модуля (ГБ)\n- Частота (МГц)\n- Латентность (CL)\n- ECC (да/нет)\n- Напряжение (В)',
    nas:        '- Кол-во отсеков для дисков\n- Поддерживаемые диски (3.5\"/2.5\", HDD/SSD)\n- Процессор (кол-во ядер, частота)\n- Оперативная память (объём)\n- Сетевые интерфейсы (Ethernet, скорость)\n- RAID (поддерживаемые уровни)\n- Протоколы (SMB, NFS, iSCSI, AFP)\n- USB-порты\n- Потребляемая мощность',
    san:        '- Форм-фактор (Rack, U)\n- Кол-во дисковых отсеков\n- Поддерживаемые накопители (HDD/SSD, интерфейс)\n- Контроллеры (кол-во, резервирование)\n- Кэш (объём)\n- Интерфейсы подключения (FC, iSCSI, SAS)\n- RAID (поддерживаемые уровни)\n- Макс. сырой объём (ТБ)\n- Управление',
    os:         '- Тип ОС (десктоп/серверная)\n- Версия / релиз\n- Поддерживаемые платформы (x86_64, ARM)\n- Тип ядра\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК (если применимо)\n- Тип лицензии (бессрочная/подписка)\n- Срок технической поддержки\n- Количество лицензий',
    office:     '- Состав пакета (текстовый редактор, электронные таблицы, презентации, почтовый клиент)\n- Совместимость форматов (OOXML, ODF)\n- Наличие в реестре Минцифры\n- Тип лицензии (бессрочная/подписка)\n- Кол-во рабочих мест\n- Поддерживаемые ОС (Windows, Linux, macOS)\n- Срок технической поддержки',
    antivirus:  '- Тип защиты (файловый антивирус, проактивная защита, веб-защита, почтовый антивирус)\n- Количество защищаемых устройств (рабочих станций / серверов)\n- Поддерживаемые ОС (Windows, Linux, macOS)\n- Централизованное управление (консоль администратора)\n- Автоматическое обновление баз\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК\n- Срок лицензии (мес)\n- Срок технической поддержки',
    dbms:       '- Тип СУБД (реляционная SQL)\n- Совместимость с PostgreSQL\n- Поддерживаемые ОС (Linux, Windows)\n- Тип лицензии (по ядрам / по серверам / подписка)\n- Кол-во ядер / серверов\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК (если применимо)\n- Функции (резервное копирование, репликация, кластеризация)\n- Срок технической поддержки',
    crypto:     '- Класс СКЗИ (не ниже КС1)\n- Поддерживаемые алгоритмы (ГОСТ Р 34.10-2012, ГОСТ Р 34.11-2012, ГОСТ 28147-89)\n- Тип поставки (ПО / аппаратный токен)\n- Количество лицензий\n- Поддерживаемые ОС\n- Сертификат ФСБ России (номер, срок)\n- Наличие в реестре Минцифры\n- Срок технической поддержки',
    vdi:        '- Количество виртуальных рабочих мест (лицензий)\n- Поддерживаемые гипервизоры\n- Поддерживаемые гостевые ОС\n- Протоколы доступа (RDP, PCoIP, SPICE, Blast)\n- Управление профилями пользователей\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК (если применимо)\n- Тип лицензии\n- Срок технической поддержки',
    virt:       '- Тип (серверная виртуализация, контейнеризация)\n- Поддерживаемые гостевые ОС\n- Макс. кол-во виртуальных машин\n- Live-миграция\n- Высокая доступность (HA, кластеризация)\n- Управление (веб-консоль)\n- Наличие в реестре Минцифры\n- Тип лицензии (по сокетам / по серверам)\n- Срок технической поддержки',
    vks:        '- Макс. кол-во участников видеоконференции\n- Качество видео (Full HD/4K)\n- Запись конференций\n- Демонстрация экрана\n- Чат, обмен файлами\n- Интеграция с календарями\n- Шифрование (TLS, SRTP)\n- Поддерживаемые платформы (Windows, Linux, macOS, мобильные)\n- Наличие в реестре Минцифры\n- Тип лицензии',
    email:      '- Макс. кол-во почтовых ящиков\n- Поддерживаемые протоколы (SMTP, IMAP, POP3, CalDAV, CardDAV)\n- Веб-интерфейс\n- Объём хранилища на ящик\n- Антиспам, антивирус\n- Шифрование (TLS, S/MIME)\n- Наличие в реестре Минцифры\n- Тип лицензии',
    ecm:        '- Тип системы (СЭД/ECM)\n- Электронный документооборот (маршруты, согласование)\n- Поддержка ЭП (электронной подписи)\n- Интеграция (1С, СМЭВ, Active Directory)\n- Кол-во пользователей (лицензий)\n- Наличие в реестре Минцифры\n- Тип лицензии',
    backup_sw:  '- Типы резервного копирования (полное, инкрементальное, дифференциальное)\n- Поддерживаемые источники (файлы, БД, ВМ, приложения)\n- Хранилища (локальное, сетевое, облачное, ленточное)\n- Шифрование резервных копий\n- Дедупликация\n- Расписание\n- Наличие в реестре Минцифры\n- Тип лицензии\n- Срок технической поддержки',
    dlp:        '- Контролируемые каналы (почта, веб, USB, мессенджеры, облака, принтеры)\n- Кол-во защищаемых рабочих мест\n- Анализ контента (лингвистический, регулярные выражения, fingerprints)\n- Режимы (мониторинг, блокировка)\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК\n- Тип лицензии',
    siem:       '- Кол-во источников событий (EPS)\n- Корреляция событий\n- Встроенные правила детектирования\n- Интеграция (syslog, WMI, SNMP, API)\n- Хранение событий (объём, срок)\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК\n- Тип лицензии',
    firewall_sw:'- Пропускная способность (Мбит/с)\n- Функции (Stateful Firewall, IPS/IDS, NAT, VPN)\n- Кол-во защищаемых узлов\n- Управление (веб-интерфейс, CLI)\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК\n- Тип лицензии',
    edr:        '- Функции (детектирование, расследование, реагирование)\n- Кол-во защищаемых конечных точек\n- Поддерживаемые ОС\n- Интеграция с SIEM\n- Централизованное управление\n- Наличие в реестре Минцифры\n- Сертификат ФСТЭК\n- Тип лицензии',
    cartridge:  '- Совместимость (модели принтеров/МФУ — через «или эквивалент»)\n- Тип (оригинальный/совместимый)\n- Цвет (чёрный/цветной)\n- Ресурс (стр, при 5% заполнении)\n- Тип (лазерный/струйный)',
...
ЗАПРЕЩЕНО (нарушает ст. 33 44-ФЗ и антимонопольное законодательство):
- Торговые марки, производители, артикулы, part numbers, SKU, модели
- Сокеты процессора (LGA, AM5 и т.д.)
- Слова «рекомендуется», «желательно», «предпочтительно» — только «должен», «не менее», «не более»
- Указание конкретных технологий без «или эквивалент» (ч. 3 ст. 33 44-ФЗ)

ОБЯЗАТЕЛЬНЫЕ правила формулировок:
- Числовые значения: «не менее X» (НЕ ">= X" и НЕ просто число)
- Тип матрицы: «IPS или эквивалент (угол обзора не менее 178°)»
- Разрешение: «не менее 1920x1080» (не точное значение)
- Единицы: ГГц, МГц, ГБ, МБ, ТБ (русские, не GHz/GB/MB)
- Тип памяти: «DDR4 или выше» (не просто DDR4)
- Интерфейсы: «USB 3.2 или выше», «Wi-Fi 6 или эквивалент»
- Вес/масса: «не более X кг» (НЕ «не менее»)

Национальный режим (ПП РФ № 1875 от 23.12.2024):
${isSW ? `- ПО ОБЯЗАТЕЛЬНО должно быть в реестре Минцифры России (ПП РФ № 1236)
- Указать класс сертификации ФСТЭК России / ФСБ России где применимо
- Для СКЗИ: «не ниже класса КС1» со ссылкой на Приказ ФСБ № 66`
: `- Для радиоэлектроники (ОКПД2 26.*): подтверждение через РЭПР (ГИСП) или евразийский реестр
- Подтверждение страны происхождения: выписка из ГИСП (НЕ «заключение Минпромторга»)
- УПД или товарная накладная (ТОРГ-12) — это разные документы`}
...
async function buildDocx(rows: GoodsRow[], lawMode: LawMode): Promise<Blob> {
  ...
  children.push(
    ...
    new Paragraph({
      children: [new TextRun({
        text: multi
          ? `на поставку товаров: ${doneRows.length} позиций`
          : `на поставку товара: ${(GOODS_CATALOG[doneRows[0].type]?.name ?? '')}`,
  ...
  // ── РАЗДЕЛ 1. ОБЩИЕ СВЕДЕНИЯ ──
  {
    ...
    const sumRows: TableRow[] = [
      new TableRow({
        children: [new TableCell({
          ...
          shading: cellShade('1F5C8B'), borders: allBorders(),
          margins: { top: 60, bottom: 60, left: 80, right: 80 },
        })],
      }),
      new TableRow({ children: [s1LabelCell('Наименование объекта поставки:'), valueCell(multi ? `Комплект товаров (${doneRows.length} позиций)` : g0.name)] }),
    ...
    if (!multi) {
      const okpd2Code = row0.meta?.okpd2_code || g0.okpd2;
      const okpd2Name = row0.meta?.okpd2_name || g0.okpd2name;
      const ktru = row0.meta?.ktru_code || g0.ktruFixed || '';
      sumRows.push(new TableRow({ children: [s1LabelCell('Код ОКПД2:'), valueCell(`${okpd2Code}${okpd2Name ? ' — ' + okpd2Name : ''}`)] }));
      if (ktru) sumRows.push(new TableRow({ children: [s1LabelCell('Код КТРУ:'), valueCell(ktru)] }));
    }
    sumRows.push(
    ...
    children.push(boldPara('1. Общие сведения'));
    children.push(new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: sumRows }));
    children.push(new Paragraph({ children: [], spacing: { after: 120 } }));
  }

  // ── РАЗДЕЛ 2. ОБЪЁМ ПОСТАВКИ ──
  children.push(boldPara('2. Объём поставки'));
  if (multi) {
  ...
  children.push(boldPara('3. Технические характеристики'));
  if (multi) {
    children.push(regPara(`Технические характеристики каждой позиции приведены в Приложениях 1–${doneRows.length} к настоящему Техническому заданию.`));
  } else {
    // Одна позиция — таблица характеристик прямо здесь
    const row0 = doneRows[0]; const g0 = GOODS_CATALOG[row0.type] ?? GOODS_CATALOG['pc'];
    const isSW0 = !!g0.isSoftware;
    const nacRegime0 = row0.meta?.nac_regime || getNacRegime(row0.type);
    const specs0 = row0.specs ?? [];
    children.push(new Paragraph({
      children: [new TextRun({ text: `${g0.name}${row0.model ? ' (' + row0.model + ')' : ''}`, bold: true, font: FONT, size: FONT_SIZE })],
    ...
    if (specs0.length > 0) {
      ...
      if (!isSW0 && (nacRegime0 === 'pp878' || nacRegime0 === 'pp616')) {
  ...
  children.push(boldPara('4. Требования к качеству поставляемого Товара'));
  if (hasHW) {
    for (const t of [
  ...
  // ── РАЗДЕЛ 5. ТРЕБОВАНИЯ СООТВЕТСТВИЯ (нацрежим) ──
  children.push(boldPara('5. Требования соответствия (национальный режим)'));
  ...
  if (regimes.has('pp1236')) {
    children.push(boldPara('Для программного обеспечения:', 80));
    children.push(regPara('ПО должно быть включено в Единый реестр российских программ (реестр Минцифры) в соответствии с ПП РФ от 16.11.2015 № 1236. Поставщик обязан представить реестровую запись (выписку) из реестра Минцифры.'));
  }
  if (regimes.has('pp878')) {
    children.push(boldPara('Для радиоэлектронной продукции:', 80));
    children.push(regPara('Товар должен быть включён в единый реестр российской радиоэлектронной продукции (РЭПР) либо евразийский реестр промышленных товаров. Подтверждение происхождения — выписка из ГИСП или реестровая запись по ПП РФ № 1875.'));
  }
  if (regimes.has('pp616')) {
    children.push(boldPara('Для промышленных товаров:', 80));
    children.push(regPara('Товар должен иметь подтверждение производства на территории государств — членов ЕАЭС. Подтверждение — документы по ПП РФ № 1875 и ПП РФ от 17.07.2015 № 719 (при применимости).'));
  }
  if (doneRows.some((r) => ['pc','laptop','monoblock','server','tablet','thinClient'].includes(r.type))) {
    children.push(regPara('Вычислительная техника должна быть совместима с отечественными операционными системами, включёнными в реестр Минцифры, или эквивалентными (ч. 3 ст. 33 44-ФЗ).'));
  }

  // ── РАЗДЕЛ 6. ГАРАНТИЯ ──
  children.push(boldPara('6. Требования к гарантии качества'));
  if (hasHW) {
    children.push(regPara(`Гарантийный срок на оборудование — не менее 12 (двенадцати) месяцев с даты подписания акта приёмки. Дата выпуска Товара — не ранее 1 января ${currentYear} г.`));
    children.push(regPara('При обнаружении дефектов в течение гарантийного срока Поставщик обязан устранить их за свой счёт в течение 30 дней с момента уведомления. При невозможности ремонта — замена Товара новым или аналогичным.'));
  }
  if (hasSW) {
  ...
  children.push(regPara('Все сопутствующие гарантийному обслуживанию мероприятия (доставка, погрузка, разгрузка) осуществляются силами и за счёт Поставщика.'));

  // ── РАЗДЕЛ 7. ПОСТАВКА ──
  children.push(boldPara('7. Место, сроки и условия поставки товара'));
  ...
  children.push(regPara('Поставщик обязан согласовать дату и время поставки не позднее чем за 2 рабочих дня. Поставка — пн–пт, 09:00–13:00, 14:00–17:00.'));

  // ── РАЗДЕЛ 8. ТАРА И УПАКОВКА ──
  if (hasHW) {
    children.push(boldPara('8. Требования к таре и упаковке'));
    children.push(regPara('Товар поставляется в заводской таре и упаковке, обеспечивающей сохранность при транспортировке. Маркировка — в соответствии с документацией производителя.'));
  }
