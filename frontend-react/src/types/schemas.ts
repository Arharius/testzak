...
export const automationSettingsSchema = z.object({
  ...
  deliveryRetries: z.number().int().min(0).max(8),
  deliveryBackoffMs: z.number().int().min(200).max(10000),
  requireHttpsForIntegrations: z.boolean(),
  tenantId: z.string().max(120),
  billingEnabled: z.boolean(),
  billingCurrency: z.string().max(8),
  billingPricePerDocCents: z.number().int().min(0).max(500000),
...
export const defaultAutomationSettings: AutomationSettings = {
  ...
  backendApiToken: '',
  deliveryRetries: 2,
  deliveryBackoffMs: 800,
  requireHttpsForIntegrations: true,
  tenantId: '',
  billingEnabled: false,
  billingCurrency: 'RUB',
  billingPricePerDocCents: 9900,
