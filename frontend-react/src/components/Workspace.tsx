...
function buildImportedSpecsPromptBlock(row: GoodsRow): string {
  ...
  if (hasDocxSpecs) {
    return [
      '',
      '════════════════════════════════════════════════════════',
      'ХАРАКТЕРИСТИКИ ИЗ ФАЙЛА ЗАКАЗЧИКА — ОБЯЗАТЕЛЬНЫЙ ИСТОЧНИК ДАННЫХ:',
      '════════════════════════════════════════════════════════',
      `У этой позиции ЕСТЬ ${importedSpecs.length} характеристик, загруженных из ТЗ-файла Заказчика.`,
      'ПРАВИЛО №1 — ВСЕ ЧИСЛА СОХРАНЯТЬ ТОЧНО: если в файле «16 ГБ», пиши «не менее 16 ГБ» (не «не менее 8 ГБ»); «17.3"» → «не менее 17,3 дюйма» (не «не менее 15 дюймов»); «4.4 ГГц» → «не менее 4,4 ГГц» (не «не менее 2.4 ГГц»).',
      'ПРАВИЛО №2 — СТРОКИ НЕ УДАЛЯТЬ И НЕ ЗАМЕНЯТЬ ШАБЛОНАМИ: каждая строка ниже должна попасть в итоговую таблицу с сохранёнными числами.',
      'ПРАВИЛО №3 — БРЕНДЫ: убрать только торговую марку (Intel, AMD, ASUS, Samsung...), числовые параметры при этом СОХРАНИТЬ.',
      'ЗАПРЕЩЕНО: генерировать список с нуля, игнорировать данные ниже, менять числа, округлять.',
      '',
...
РЕЖИМ ЗАКУПКИ: ${law}. Если режим «44‑ФЗ», действуй максимально строго: не указывай бренды и модели, не включай коды ОКПД2, КТРУ и другие классификаторы в текст описания; если режим «223‑ФЗ», допустимо немного больше гибкости, но также нельзя необоснованно ограничивать конкуренцию.

Коды ОКПД2, КТРУ и другие классификаторы не включай в текст описания объекта закупки и в таблицу характеристик; считай, что система обрабатывает их отдельно во внутреннем «Паспорте публикации». Если в исходном тексте есть такие коды, игнорируй их в финальном ТЗ.

Все упоминания брендов, торговых марок и конкретных моделей (например, «ПК Брест», «ALD Pro Server») нужно заменить на нейтральные формулировки уровня категории/класса оборудования («персональный компьютер», «серверное оборудование», «вычислительное устройство» и т.п.) с «или эквивалент» при необходимости.

БЛОК ОБРАБОТКИ ИМПОРТИРОВАННЫХ ХАРАКТЕРИСТИК
Если у позиции ЕСТЬ импортированные характеристики из DOCX (JSON imported specs ниже):
У ЭТОЙ ПОЗИЦИИ УЖЕ ЕСТЬ ХАРАКТЕРИСТИКИ, ЗАГРУЖЕННЫЕ ИЗ ТЗ‑ФАЙЛА ЗАКАЗЧИКА (JSON imported specs ниже).
ОБЯЗАТЕЛЬНОЕ ПРАВИЛО: используй ИМЕННО ЭТИ ХАРАКТЕРИСТИКИ как основу итоговой таблицы технических характеристик.
Не придумывай альтернативный список характеристик и не заменяй их на обобщённые или шаблонные формулировки по категории товара.
Допустимо:
– добавить «не менее»/«не более»/«или эквивалент» к числовым значениям согласно требованиям 44‑ФЗ / 223‑ФЗ;
– привести единицы измерения к единой системе (ГБ, МГц, мм, Вт·ч и т.п.);
– убрать ТОЛЬКО торговую марку/бренд из значения (Intel, AMD, Samsung, ASUS и т.п.), сохраняя все числовые параметры этого значения;
– объединять явно дублирующие строки без потери смысла;
– если знак ≥/≤ уже есть в значении, сохрани его как «не менее»/«не более» с тем же числом.
Запрещено — КРИТИЧЕСКИ ВАЖНО:
– ИЗМЕНЯТЬ ЧИСЛА: если в источнике «16 ГБ» — пиши «не менее 16 ГБ», НЕЛЬЗЯ «не менее 8 ГБ»; если «17.3 дюйма» — пиши «не менее 17,3 дюйма», НЕЛЬЗЯ «не менее 15 дюймов»;
– ОКРУГЛЯТЬ или ОБОБЩАТЬ числовые значения (например, 4.4 ГГц → 4.0 ГГц — ЗАПРЕЩЕНО);
– УДАЛЯТЬ строки из этого списка;
– ЗАМЕНЯТЬ конкретные характеристики шаблонными («многоядерный процессор» вместо «не менее 10 ядер»);
– ИГНОРИРОВАТЬ импортированные данные и генерировать список с нуля;
– ДОБАВЛЯТЬ новые строки вместо существующих.
Пример корректной трансформации: значение «Intel Core i5-1235U, 10 ядер, 3.3/4.4 ГГц, 12 МБ кеш» → «не менее 10 ядер, базовая частота не менее 3,3 ГГц, турбочастота не менее 4,4 ГГц, кеш не менее 12 МБ» (бренд убран, все числа сохранены).
Итоговая таблица должна содержать тот же набор характеристик, что и в JSON imported specs, только в нормализованном и юридически корректном виде.

Если у позиции НЕТ импортированных характеристик из DOCX (список пустой):
Для этой позиции нет импортированных характеристик из файла заказчика.
Сформируй перечень технических характеристик на основе типа товара и описания ниже так, чтобы по этим характеристикам могли быть предложены товары не менее чем от двух разных производителей.
Используй нейтральные формулировки, избегай брендов и моделей, добавляй «не менее»/«не более» и «или эквивалент» там, где это нужно для соблюдения 44‑ФЗ и 223‑ФЗ.
Все характеристики должны быть измеримыми и проверяемыми (конкретные числа, диапазоны, ссылки на общепринятые стандарты, но не на конкретные коммерческие линейки).
    ...
    <section className="panel">
      ...
      <div className="workspace-side-card workspace-composer-card">
        ...
        <div className="workspace-chip-row workspace-chip-row--dense">
          ...
          <span className="workspace-mini-chip">Build: {APP_BUILD_LABEL}</span>
        ...
        <div className="workspace-composer-stack">
          ...
          <div className="workspace-inline-note">
            Чтобы вручную добавить позицию, нажмите «+ Свой товар» или выберите «Свой товар (ручной ввод)» в каталоге, затем напишите описание: <em>телевизор 55 дюймов</em>, <em>масло для шредера</em>, <em>монтаж ЛВС</em>.
          </div>
          <div className="workspace-chip-row workspace-chip-row--dense">
        ...
        <div className="workspace-action-grid workspace-action-grid--toolbar">
          ...
          <button type="button" onClick={resetWorkspaceDraft}>↺ Очистить черновик</button>
          <input
      ...
      <Suspense fallback={<div className="workspace-inline-note">Загружаю контроль публикации…</div>}>
        <WorkspaceReviewSections
          showPublicationControl={hasPublicationBaseline && liveLegalSummarySourceRows.length > 0}
          publicationStatusTone={publicationStatusTone}
          publicationStatusLabel={publicationStatusLabel}
          readinessGate={readinessGate}
