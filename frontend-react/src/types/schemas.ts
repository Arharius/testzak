...
export const platformIntegrationSchema = z.object({
  ...
  customerInn: z.string().max(12).default(''),
  customerKpp: z.string().max(9).default(''),
  orgName: z.string().max(500).default(''),
  orgNameShort: z.string().max(300).default(''),
  customerAddress: z.string().max(500).default(''),
  deliveryAddress: z.string().max(500).default(''),
  deliveryDays: z.number().min(1).max(365).default(60),
  contactPersonName: z.string().max(200).default(''),
  contactPersonTitle: z.string().max(200).default(''),
  contactPersonPhone: z.string().max(50).default(''),
  contactPersonEmail: z.string().max(200).default(''),
  approvalPersonName: z.string().max(200).default(''),
  approvalPersonTitle: z.string().max(200).default(''),
  industryPreset: z.enum([
    'general',
    'it_department',
    'education',
    'healthcare',
  ...
  organizationInstructions: z.string().max(3000).default(''),
  defaultWarrantyMonths: z.number().min(0).max(120).default(0),
...
export const enterpriseSettingsSchema = z.object({
  ...
  blockExportsOnFail: z.boolean().default(false),
  blockIntegrationsOnFail: z.boolean().default(false),
