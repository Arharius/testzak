...
check('isDocxSummaryTable принимает формат с qty-колонкой (без коммерческих полей)',
  rowImport.includes("const hasQty = headers.some((cell) => HEADER_ALIASES.qty.includes(cell))") &&
  rowImport.includes("return hasName && hasOkpd2 && hasAppendix && (hasCommercial || hasQty)"));
...
check('isDocxSummaryTable требует наименование + окпд2 + прил',
  rowImport.includes('const hasName = headers.includes') &&
  rowImport.includes("const hasOkpd2 = headers.includes('окпд2')") &&
...
check('RUNTIME: сводная таблица ТЗ-генератора (5 колонок) распознаётся',
  isDocxSummaryTableLocal([
    ['№', 'Наименование', 'Кол-во', 'ОКПД2', 'Прил.№'],
    ['1', 'Системный блок', '30 шт.', '26.20.15.000', 'Прил. 1'],
  ]));

check('RUNTIME: сводная таблица с коммерческими полями (7 колонок) распознаётся',
  isDocxSummaryTableLocal([
    ['№', 'Наименование', 'Тип лицензии', 'Срок действия', 'Кол-во', 'ОКПД2', 'Прил.№'],
    ['1', 'Microsoft Office', 'Бессрочная', '—', '10', '58.29.29.000', 'Прил. 1'],
  ]));

check('RUNTIME: сводная таблица с "Количество" вместо "Кол-во" распознаётся',
  isDocxSummaryTableLocal([
    ['Наименование', 'Количество', 'ОКПД2', 'Прил.№'],
    ['Монитор', '60', '26.20.17.110', 'Прил. 2'],
  ]));
...
check('RUNTIME: таблица без ОКПД2 НЕ является сводной',
  !isDocxSummaryTableLocal([
    ['Наименование', 'Кол-во', 'Прил.№'],
    ['Монитор', '60', '2'],
  ]));

check('RUNTIME: таблица только с наименованием (нет окпд2/прил/qty) НЕ является сводной',
  !isDocxSummaryTableLocal([
