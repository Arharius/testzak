...
export function buildSection2Rows(
  ...
  const specialMap: Record<string, Row2[]> = {
    ...
    ldap:       [
      ...
      ['Миграция и централизованное управление', 'Служба каталогов должна обеспечивать централизованное управление пользователями и ПК, поддерживать групповые политики, автоматизированную установку ОС по сети и миграцию из Microsoft Active Directory'],
      ['Лицензирование', 'Лицензионная схема должна предусматривать серверную лицензию (контроллер домена) и клиентские лицензии / лицензии на управляемые объекты'],
    ],
    osSupport:   [
...
export function buildSection4Rows(goodsType: string, lawMode: LawMode): Row2[] {
  ...
  if (goodsType === 'osSupport' || goodsType === 'supportCert') {
    return [
      ...
      ['4.3. Сервисные обязательства', 'Поставщик обеспечивает активацию выбранного уровня поддержки, передачу SLA и каналов обращения, а также контроль корректности регистрации Заказчика в системе правообладателя'],
      ['4.4. Обновления и база знаний', 'В течение срока действия сертификата Заказчику предоставляется доступ к обновлениям, патчам безопасности, базе знаний и иным материалам, предусмотренным соответствующим уровнем поддержки'],
      ['4.5. Подтверждающие документы', 'Поставщик передаёт сертификат / подтверждение активации, регистрационный номер поддержки, условия SLA и иные документы правообладателя на русском языке'],
    ];
  }
