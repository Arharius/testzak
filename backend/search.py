...
def _build_extraction_prompt(product: str, goods_type: str, is_software: bool) -> str:
    ...
    common_rules = (
        "\n\nПРАВИЛА ПРЕОБРАЗОВАНИЯ НАЙДЕННЫХ ХАРАКТЕРИСТИК В ТЗ:\n"
        "Найдено в datasheet           → В ТЗ пишем:\n"
        "'Процессор Intel Core i5'     → 'тактовая частота не менее X ГГц, количество ядер не менее N'\n"
        "'RAM 16 GB DDR5'              → 'Объём оперативной памяти: не менее 16 ГБ'\n"
        "'SSD 512 GB NVMe'             → 'Накопитель: SSD, объём не менее 512 ГБ, интерфейс NVMe или SATA'\n"
        "'Цвет: серебристый'           → НЕ включать (если не функционально обусловлен)\n"
        "'Windows 11 Pro'              → 'Совместимость с Astra Linux, РЕД ОС, Alt Linux или эквивалент'\n"
        "'Гарантия производителя 1 год'→ 'Гарантийный срок: не менее 12 мес.'\n"
        "\nАЛГОРИТМ ВЫБОРА ХАРАКТЕРИСТИК:\n"
        "1. КТРУ (каталог zakupki.gov): если позиция есть — используй КТРУ-характеристики как основу\n"
        "2. ГОСТ и стандарты: если есть действующий ГОСТ — ссылайся на него в значении\n"
        "3. Datasheet производителя: реальные цифры → преобразуй в 'не менее X'\n"
        "4. Аналогичные ТЗ на zakupki.gov: стандартный набор параметров для данного товара\n"
        "\nОБЩИЕ ПРАВИЛА:\n"
        "- name — название параметра по-русски\n"
        "- value — значение с 'не менее X' / 'не более X' / 'от X до Y' для числовых характеристик\n"
        "- unit — единица измерения или пустая строка\n"
        "- Диапазоны: 'от -40 до +60' (не точечные значения без обоснования)\n"
        "- Бренды/торговые марки: добавляй 'или эквивалент'\n"
        "- Совместимость ПО: перечислением с 'или эквивалент': 'Astra Linux, РЕД ОС, Alt Linux или эквивалент'\n"
        "- Материалы: не торговые названия — 'твёрдый сплав (карбид вольфрама WC или аналог)' вместо 'победит'\n"
        "- Цвет: не включать если не обусловлен функцией (маскировка, корпоративный стандарт с обоснованием)\n"
        "- ЗАПРЕЩЕНО: name 'Модель', 'Бренд', 'Производитель', 'Артикул', 'SKU'\n"
        "- ЗАПРЕЩЕНО: value 'не указан', 'не указано', 'н/д', 'неизвестно', '—'\n"
        "- ЗАПРЕЩЕНО: emoji, внутренние комментарии, пометки 'требуется проверка', 'при применимости уточните'\n"
        "- Если данных нет — вернуть пустой массив []\n"
        "- Лучше 5 точных параметров чем 15 сомнительных\n"
        "\nКРИТИЧЕСКИ ВАЖНО — НАЗВАНИЯ ЧИПСЕТОВ И КОНТРОЛЛЕРОВ:\n"
        "НИКОГДА не включать в ТЗ торговые наименования чипсетов — только функциональные параметры:\n"
        "Intel I219V, I225V, I226V → 'сетевой контроллер со скоростью не менее 1 Гбит/с'\n"
        "Realtek ALC897, ALC662, ALC255 → 'аудиокодек, совместимый со стандартом High Definition Audio'\n"
        "Intel AX211, AX201, AX200 → 'модуль беспроводной связи стандарта не ниже Wi-Fi 6E (802.11ax)'\n"
        "Intel AX411 → 'модуль беспроводной связи стандарта не ниже Wi-Fi 6E (802.11ax) с поддержкой Bluetooth 5.x'\n"
        "Intel PTT / AMD fTPM → 'программный модуль доверенной платформы (TPM) версии не ниже 2.0'\n"
        "Intel Thunderbolt 4 → 'интерфейс высокоскоростной передачи данных не менее 40 Гбит/с (USB4/Thunderbolt)'\n"
        "Любой GPU-чипсет → 'видеоадаптер с видеопамятью не менее X ГБ, поддержка DirectX 12 или OpenGL 4.x'\n"
        "ПРАВИЛО: если в datasheet указан конкретный чипсет/контроллер — преобразуй в функциональный параметр по образцу выше.\n"
    )

    if is_software:
        return (
            "Ты — генератор технических характеристик для государственных закупок РФ (44-ФЗ, 223-ФЗ). "
            ...
            "3. Совместимость: поддерживаемые ОС (с 'или эквивалент'), СУБД, платформы виртуализации\n"
            "4. Безопасность: сертификаты ФСТЭК/ФСБ (класс не ниже X), мандатный контроль, шифрование\n"
            "5. Управление: централизованное управление, веб-консоль, API\n"
            "6. Масштабирование: макс. пользователей (не менее X), лицензирование, кластеризация\n"
            "7. Интеграция: поддерживаемые протоколы, форматы, API\n"
            "8. Документация и поддержка: русскоязычная документация, техподдержка\n"
            "Извлекай ВСЕ характеристики — минимум 20 для ПО."
            + common_rules
        )
    else:
        return (
            ...
            "Формат ответа — ТОЛЬКО JSON-массив, без лишнего текста:\n"
            '[{"name": "Процессор", "value": "тактовая частота не менее 3,0 ГГц, количество ядер не менее 6", "unit": ""},\n'
            ' {"name": "Оперативная память", "value": "не менее 16", "unit": "ГБ"}, ...]\n\n'
            "ОБЯЗАТЕЛЬНЫЕ ГРУППЫ характеристик для оборудования:\n"
            "1. Основные параметры: процессор (частота, ядра), память (объём), накопитель (объём, интерфейс)\n"
            "2. Дисплей/экран (если применимо): диагональ не менее X дюйм, разрешение не менее X, тип матрицы\n"
            "3. Интерфейсы: порты USB (тип, количество не менее X), видеовыходы, сетевые разъёмы\n"
            "4. Сетевые возможности: Ethernet (скорость не менее X Мбит/с), Wi-Fi (стандарт), Bluetooth\n"
            "5. Конструктив: размеры (не более X мм), вес (не более X кг), тип корпуса\n"
            "6. Питание: мощность БП не менее X Вт, аккумулятор не менее X Вт·ч\n"
            "7. Комплектация: что входит в комплект поставки\n"
            "Извлекай ВСЕ характеристики — минимум 15 для оборудования."
            + common_rules
        )
...
def _ai_generate_model_specs(product: str, goods_type: str = "") -> list[dict]:
    ...
    system_prompt = (
        "Ты формируешь технические характеристики для государственных закупок (44-ФЗ РФ) в антикоррупционном стиле."
        " Верни только JSON-массив объектов {name, value, unit} без markdown и без пояснений.\n"
        "ПРАВИЛА ANTI-ФАС (обязательно):\n"
        "1. Для числовых параметров используй ТОЛЬКО формулировку 'не менее X' или 'не более X' — никогда точное число.\n"
        "2. Для разрядных стандартов (USB, HDMI, Bluetooth) указывай конкретную версию с цифрой (USB 3.0, Bluetooth 5.0).\n"
        "3. Не упоминай торговые марки, бренды, названия производителей в значениях характеристик.\n"
        "4. Запрещены фразы: 'по требованиям заказчика', 'по типу товара', 'по документации производителя',"
        " 'в количестве, достаточном для эксплуатации', 'при необходимости', 'по согласованию'.\n"
        "5. Запрещены характеристики: Модель, Бренд, Производитель, Артикул, SKU, Гарантия, Упаковка, Маркировка.\n"
        "6. Каждое значение должно быть измеримым и проверяемым — без оценочных прилагательных ('высокая', 'хорошая')."
    )
    user_prompt = (
        f"Товар: {product}\n"
        f"Тип товара: {type_hint or goods_type or 'товар'}\n\n"
        f"Нужно вернуть не менее {minimum} технических характеристик.\n"
        "Для оборудования обязательно включи: интерфейсы подключения (с версиями), скорость передачи данных"
        " (не менее X Мбит/с), ёмкость/объём (не менее X ГБ), разрешение/скорость (не менее X), размеры, вес.\n"
        "Для ПО включи: тип лицензии, архитектуры, модули, протоколы интеграции, совместимые ОС.\n"
        "Не указывай модель, бренд, артикул, производителя или гарантийные условия.\n"
        "Если значение неизвестно — просто пропусти характеристику."
    )
    payload = {
        "model": model,
        "messages": [
