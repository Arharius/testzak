...
function parseGroupedSpecText(text: string): SpecItem[] {
  const specs: SpecItem[] = [];
  let group = 'Общие сведения';
  for (const rawLine of String(text || '').split('\n')) {
    const line = rawLine.trim();
    if (!line) continue;
    if (line.startsWith('[') && line.endsWith(']')) {
      group = line.slice(1, -1).trim() || group;
      continue;
    }
...
Тип лицензии: ${getFilledCommercialValue(commercial.suggestedLicenseType, 'Клиентская часть (CAL)')}
Срок действия лицензии: ${term}
...
[Лицензирование]
Метрика лицензирования: ${metric}
Лицензируемый объект: ${object}
Количество лицензий: не менее ${row.qty}
Способ поставки: Электронная
Документация на русском языке: Да
Наличие в Едином реестре российского ПО Минцифры России: Да
Обновления и исправления безопасности: Наличие в период действия поддержки
...
Массовое применение политик безопасности: Наличие
Поддержка доменной аутентификации: Наличие
Поддержка доменных сценариев входа: Наличие
Поддержка централизованного применения настроек и конфигураций к рабочим станциям и серверам в домене: Наличие
[Совместимость]
Совместимость с серверной частью: ALD Pro Server / контроллер домена или эквивалентная серверная часть службы каталогов
...
Интеграция с доменной политикой: Совместимость с групповыми политиками, OU-иерархией и механизмами централизованного управления домена
[Безопасность]
Ролевая модель доступа (RBAC): Наличие
Разграничение прав доступа: Наличие
Журналирование системных событий: Наличие
Журналирование событий безопасности: Наличие
Аудит действий администраторов: Наличие
Шифрование трафика управления: TLS 1.2 и выше
[Эксплуатация]
Веб-интерфейс администрирования: Наличие административной консоли или иного штатного интерфейса управления
API / средства автоматизации: REST API, CLI, webhooks или эквивалентные средства интеграции и автоматизации
Резервное копирование конфигурации: Наличие
Масштабирование и отказоустойчивость: Поддержка масштабирования и отказоустойчивой схемы развёртывания в рамках редакции поставки
...
Тип лицензии: ${getFilledCommercialValue(commercial.suggestedLicenseType, 'Серверная часть')}
Срок действия лицензии: ${term}
Назначение: Централизованное управление парком ПК, пользователей и серверов
[Лицензирование]
Метрика лицензирования: На экземпляр контроллера домена
Лицензионный состав: Серверная лицензия на контроллер домена и клиентские лицензии CAL на управляемые объекты
...
Обновления и исправления безопасности: Наличие в период действия поддержки
[Функциональные возможности]
Управление организационными единицами (OU): Поддержка иерархии подразделений и делегирования административных полномочий
Управление сайтами и топологией репликации: Поддержка сайтов, межсайтовых связей и настройки топологии репликации каталога
Групповые политики: Централизованное применение групповых политик и конфигураций на базе SaltStack или эквивалентного механизма
Автоматизированная установка ОС по сети: Поддержка PXE / netboot для сетевого развёртывания рабочих станций и серверов
Миграция из Microsoft Active Directory: Поддержка переноса домена, организационной структуры, пользователей и групп с сохранением структуры объектов
...
[Инфраструктура]
Репликация каталога: multi-master репликация каталога с поддержкой межсайтовой синхронизации
Поддержка межсайтовой синхронизации: Наличие
Интеграция с DHCP / DNS: Интеграция со службами DHCP и DNS, включая обслуживание записей и доменной инфраструктуры
Поддержка DNS-зон: Поддержка прямых и обратных зон DNS, необходимых для доменной инфраструктуры
...
Совместимость с клиентской частью: Клиентские лицензии CAL на устройства или пользователей
[Эксплуатация]
API / средства автоматизации: REST API, CLI, webhooks или эквивалентные средства интеграции и автоматизации
Масштабирование и отказоустойчивость: Поддержка масштабирования и отказоустойчивой схемы развёртывания в рамках редакции поставки
Техническая поддержка: По сроку поставки и условиям договора
...
Исполнение / уровень защищённости: редакция с усиленными встроенными средствами защиты информации
...
Тип ядра: монолитное
Поддерживаемые локальные файловые системы: ext4, XFS, Btrfs или эквивалентные
...
Графическая оболочка: Наличие графического пользовательского интерфейса для рабочих мест
Файловый менеджер: Наличие графического файлового менеджера
Поддержка нескольких мониторов: Наличие
Поддержка HiDPI-дисплеев: Наличие
...
Контроль целостности загрузки и исполняемых файлов: Наличие
...
Доменная аутентификация: Поддержка LDAP, Kerberos и интеграции со службой каталогов ALD Pro или эквивалентной
Поддержка DHCP-клиента: Наличие
Поддержка DNS-клиента: Наличие
[Администрирование]
Удалённое администрирование: SSH
Система управления пакетами: apt/dpkg или эквивалентная
Средства централизованного обновления: Наличие
Средства централизованного администрирования: Поддержка сценариев автоматизации и централизованного управления конфигурацией
Средства резервного копирования пользовательских настроек: Наличие встроенных или штатно поддерживаемых средств
[Виртуализация и контейнеры]
Поддержка виртуализации: KVM, QEMU или эквивалентные средства
Поддержка контейнеризации: LXC, Docker или эквивалентные средства
[Совместимость]
Совместимость с отечественными СКЗИ: Поддержка средств электронной подписи и криптографической защиты, включая КриптоПро CSP или эквивалентные
Совместимость с офисными пакетами из реестра Минцифры: Наличие
Совместимость с экосистемой Astra: ALD Pro, Брест, Termidesk, RuPost, RuBackup или эквивалентные решения
Поддержка печати: CUPS или эквивалентная подсистема печати
Поддержка сканирования: SANE или эквивалентная подсистема
Поддержка веб-браузеров: Совместимость с Яндекс.Браузером, Chromium или эквивалентными браузерами
[Сертификация и лицензирование]
Наличие в Едином реестре российского ПО Минцифры России: Наличие
Сертификат ФСТЭК России: Наличие действующего сертификата ФСТЭК России
Документация на русском языке: Руководство пользователя и руководство администратора в электронном виде
...
Тип лицензии: ${getFilledCommercialValue(commercial.suggestedLicenseType, 'На почтовый ящик / пользователя')}
Срок действия лицензии: ${term}
Количество лицензий: не менее ${row.qty}
[Лицензирование]
...
Импорт / миграция из Microsoft Exchange: Наличие с сохранением почтовых сообщений, календарей и адресных книг
API / средства интеграции: Наличие
Экспорт данных и журналов: Наличие механизмов экспорта данных, отчётов и журналов в открытых форматах
[Безопасность]
Ролевая модель доступа (RBAC): Наличие
Аудит действий администраторов: Наличие
Журналирование системных событий: Наличие
...
Тип лицензии: ${getFilledCommercialValue(commercial.suggestedLicenseType, 'Серверная часть + агенты')}
Срок действия лицензии: ${term}
Количество лицензий: не менее ${row.qty}
[Лицензирование]
...
Тип лицензии: ${getFilledCommercialValue(commercial.suggestedLicenseType, 'На физический процессор (socket)')}
Срок действия лицензии: ${term}
Количество лицензий: не менее ${row.qty}
[Лицензирование]
...
Тип лицензии: ${getFilledCommercialValue(commercial.suggestedLicenseType, 'Конкурентные пользователи (CCU)')}
Срок действия лицензии: ${term}
Количество лицензий: не менее ${row.qty}
[Лицензирование]
...
function buildDocumentSectionBundle(doneRows: GoodsRow[], lawMode: LawMode): DocumentSectionBundle {
  ...
  if (!multi) {
    ...
    pushSection1(`Требования к количеству поставляемого Товара: ${row0.qty} (${numText(row0.qty)}) ${goods0.isSoftware ? 'лицензий' : 'штук'}.`);
    ...
    pushSection1(`Перечень поставляемого Товара и сопутствующих параметров приведён в сводной таблице и приложениях № 1–${doneRows.length}.`);
  }
  ...
  pushSection2(`Цена товара не должна превышать цену, установленную ${contractWord}м, и может изменяться в пределах, установленных законодательством Российской Федерации.`);

  const section3Rows: SectionTableRow[] = [
    {
      label: '3.1',
      value: hasSW && !hasHW
...
async function buildDocx(rows: GoodsRow[], lawMode: LawMode): Promise<Blob> {
  ...
  const docSections = buildDocumentSectionBundle(doneRows, lawMode);
  const {
    currentYear,
    objectName,
    multi,
...
Ответ СТРОГО в JSON без пояснений:
{
  ...
  const renderPreview = () => {
    ...
    const renderSectionTable = (rowsData: SectionTableRow[], headers: [string, string] = ['Пункт', 'Содержание']) => (
    ...
    return (
      <div className="tz-preview" style={{ marginTop: 24, fontSize: 12, fontFamily: 'Times New Roman, serif', lineHeight: 1.5, color: '#F5F0E8' }}>
        ...
        <div style={{ ...boldStyle, fontSize: 13 }}>3. Требования к пуско-наладочным работам</div>
        {renderSectionTable(docSections.section3Rows)}

        <div style={{ ...boldStyle, fontSize: 13 }}>4. Требования к сроку предоставления гарантии качества</div>
        {renderSectionTable(docSections.section4Rows)}

        <div style={{ ...boldStyle, fontSize: 13 }}>5. Требования к таре и упаковке товара</div>
        {renderSectionTable(docSections.section5Rows)}

        <div style={{ ...boldStyle, fontSize: 13 }}>6. Место, сроки и условия поставки товара</div>
