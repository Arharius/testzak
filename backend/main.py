...
_BRAND_AVOIDANCE_INSTRUCTION = (
    ...
    "\nСТОП-ЛИСТ (автоматически применять к каждому ответу):\n"
    "• Артикулы в скобках (72821), (9421) — удалить из наименования и характеристик\n"
    "• Вес нетто/брутто, размеры упаковки — не указывать\n"
    "• SKU, арт., код 1С — удалить\n"
    "• Бренды чипсетов: Intel I219V → «сетевой контроллер ≥ 1 Гбит/с»; "
    "Realtek ALC897 → «аудиокодек HDA»; Intel AX211 → «Wi-Fi ≥ 802.11ax (6E)»\n"
    "• Отрицательные характеристики («антистатик: нет», «подсветка: нет») — не упоминать\n"
    "• Для услуг: ПП РФ №1875 НЕ применять, nac_regime = 'none'\n"
)
...
2. "legal" — ЮРИДИЧЕСКИЕ НЕТОЧНОСТИ:
   • Требование выписки из реестра Минцифры вместо указания номера реестровой записи в заявке
   • Устаревшая терминология «контроль отсутствия НДВ» — заменять на «требования к уровню доверия не ниже 4-го уровня»
   • Ограничение по дате релиза «выпущен не ранее чем за 12 месяцев» — заменять на «актуальная стабильная версия, официально поддерживаемая производителем на момент поставки»
   ...
   • Формулировка «Цена товара не должна превышать» вместо «Цена Договора является твёрдой»
   • Жёсткая привязка к конкретной ОС без «или эквивалент» и обоснования
   • Отсутствие требования лицензии ФСТЭК при настройке СЗИ в составе ПНР

3. "technical" — ТЕХНИЧЕСКИЕ/ЛОГИЧЕСКИЕ ОШИБКИ:
   • 60 дней на электронную поставку ПО (нужно разделять: передача лицензий — 10 р.д., ПНР — 50 р.д.)
