...
const CLAUSE_NUM_RE_LOCAL = /^(\d+\.(\d+\.?)*)\s*$/;

function normalizeCell(v) {
  return String(v || '').replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim();
}

function isDocxClauseTableLocal(rows) {
  ...
  const clauseCount = dataRows.filter((row) => CLAUSE_NUM_RE_LOCAL.test(normalizeCell(row[0] || ''))).length;
...
// ── isDocxClauseTable / isSpecTable защита от таблиц разделов ────────────────
check('isDocxClauseTable определена до isSpecTable (нет forward reference)',
  rowImport.indexOf('function isDocxClauseTable') < rowImport.indexOf('function isSpecTable'));

check('isSpecTable вызывает isDocxClauseTable в начале (early exit)',
  rowImport.includes('if (isDocxClauseTable(rows)) return false;'));

check('looksLikeHeaderlessSpecTable отклоняет таблицы с ≥40% пунктонумерованных ячеек',
  rowImport.includes('clauseLikeCells / sample.length >= 0.4'));

check('CLAUSE_NUM_RE учитывает паттерны "1.", "1.1", "2.3.4"',
  rowImport.includes('const CLAUSE_NUM_RE = /^(\\d+\\.(\\d+\\.?)*)\\s*$/'));

check('RUNTIME: isDocxClauseTable → true для "Пункт | Содержание"',
  isDocxClauseTableLocal([
    ['Пункт', 'Содержание'],
    ['1.', 'Общие положения'],
    ['1.1', 'Наименование объекта поставки'],
    ['1.2', 'Заказчик'],
    ['1.3', 'Исполнитель'],
    ['1.4', 'Место поставки'],
  ]));

check('RUNTIME: isDocxClauseTable → true для нумерованной таблицы без заголовка',
  isDocxClauseTableLocal([
    ['Раздел', 'Текст'],
    ['1.1', 'Общие сведения'],
...
check('RUNTIME: isDocxClauseTable → false для таблицы спецификации (2 строки данных)',
  !isDocxClauseTableLocal([
...
check('RUNTIME: isDocxClauseTable → false для сводной таблицы',
  !isDocxClauseTableLocal([
    ['№', 'Наименование', 'Кол-во', 'Прил.№'],
    ['1', 'Системный блок', '30 шт.', 'Прил. 1'],
    ['2', 'Монитор', '30 шт.', 'Прил. 2'],
  ]));

check('RUNTIME: isDocxClauseTable → false для таблицы с менее 3 строками',
  !isDocxClauseTableLocal([
    ['Пункт', 'Содержание'],
    ['1.1', 'Только одна строка'],
  ]));
