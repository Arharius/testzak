...
function buildDocumentSectionBundle(
  ...
  if (regimes.has('pp878')) {
    pushSection2('Товар должен иметь подтверждение производства промышленной продукции на территории Российской Федерации (номера реестровых записей из реестра российской промышленной продукции или евразийского реестра промышленной продукции) в соответствии с ПП РФ от 23.12.2024 № 1875 и ПП РФ от 17.07.2015 № 719 (при применимости).');
  }
  ...
  if (serviceOnly) {
    ...
    if (hasHW) {
    pushSection4('Поставщик обязан предоставить Заказчику документ (гарантийный талон, сертификат или иной документ), подтверждающий гарантийные обязательства производителя Товара на срок не менее 12 (двенадцати) месяцев, если иной срок не указан в Спецификации (Приложениях) к конкретной позиции Товара.');
    }
    if (hasSW) {
  ...
  if (serviceOnly) {
    ...
    if (hasSW && hasHW) {
      ...
      pushSection6(`Выполнение пуско-наладочных работ — в течение 50 (пятидесяти) рабочих дней с даты передачи лицензий.`);
    } else if (hasSW) {
      pushSection6(`Передача неисключительных прав (электронных лицензий) — в течение 10 (десяти) рабочих дней с даты заключения ${contractWord}.`);
      pushSection6(`Выполнение пуско-наладочных работ (при наличии) — в течение 50 (пятидесяти) рабочих дней с даты передачи лицензий.`);
    } else {
...
function buildBatchPrompt(rows: GoodsRow[], lawMode: LawMode): string {
  ...
  return `Ты эксперт по госзакупкам (${law}). Сгенерируй технические характеристики для каждого товара. Правила: измеримые параметры, «не менее X»/«не более X», без брендов, ≥2 производителей.
...
• ЗАПРЕЩЕНО: «по типу товара», «согласно назначению», «по требованию заказчика» и подобные размытые формулировки.
...
Ответ СТРОГО в JSON без пояснений:
{
            ...
            try {
              ...
              await Promise.allSettled(batchAbleIs.map(async (ri, j) => {
                ...
                if (item && item.specs && item.specs.length > 0) {
                  ...
                  next[ri] = { ...currentRow, status: 'error', error: 'не найдено в пакете' };
                  sourceStats.error += 1;
                }
                completedCount++;
              }));
            } catch (batchErr) {
              const msg = batchErr instanceof Error ? batchErr.message : 'ошибка пакетной генерации';
              for (const ri of batchAbleIs) {
                next[ri] = { ...next[ri], status: 'error', error: normalizeRowFailureMessage(next[ri], msg, 'ai') };
                sourceStats.error += 1;
                completedCount++;
              }
            }
  ...
  const processSteps = useMemo(() => {
    ...
    const isReadyToPublish = hasSpecs && readinessGate.status === 'ok';
      ...
      <div className="workspace-side-card workspace-composer-card">
        ...
        <div className="workspace-action-grid workspace-action-grid--toolbar">
              ...
              : mutation.isPending
                ...
                : publicationAutopilotRunning
                  ...
                  : !canUseAiAssist && !canRunGenerationWithoutAi
                    ? 'Нет доступа к AI'
                    : !canUseAiAssist && canRunGenerationWithoutAi
                      ? 'Подготовить характеристики из файла'
          ...
          <button
            ...
            title={paymentRequired ? 'Trial завершён: оформите Pro Business' : !canUseAiAssist ? 'Требуется вход для поиска' : 'Система найдёт реальные характеристики по модели в открытых источниках'}
          >
            {internetSearching ? '⏳ Ищу характеристики...' : paymentRequired ? '🔒 Поиск закрыт' : '🌐 Найти характеристики в интернете'}
          </button>
          <button
            type="button"
            onClick={() => void searchZakupki()}
            disabled={paymentRequired || eisSearching || !canUseAiAssist || publicationAutopilotRunning}
            title={paymentRequired ? 'Trial завершён: оформите Pro Business' : !canUseAiAssist ? 'Требуется вход для поиска в ЕИС' : 'Найдёт похожие закупки в Единой информационной системе'}
          >
            {eisSearching ? '⏳ Ищу в ЕИС...' : paymentRequired ? '🔒 ЕИС закрыт' : '🏛️ Найти похожие ТЗ в ЕИС'}
          </button>
        </div>
        {docxReady && !paymentRequired && !publicationAutopilotRunning && (
          <div className="workspace-review-cta">
            ...
            >
              🔍 Проверить ТЗ на соответствие ФАС
            </button>
