...
function buildDocumentSectionBundle(
  ...
  if (regimes.has('pp878')) {
    pushSection2('Товар должен иметь подтверждение производства промышленной продукции на территории Российской Федерации (номера реестровых записей из реестра российской промышленной продукции или евразийского реестра промышленной продукции) в соответствии с ПП РФ от 23.12.2024 № 1875.');
  }
  ...
  if (serviceOnly) {
    ...
    if (hasHW) {
    pushSection4('Поставщик обязан предоставить Заказчику документ (гарантийный талон, сертификат или иной документ), подтверждающий гарантийные обязательства производителя Товара на срок не менее 12 (двенадцати) месяцев.');
    }
    if (hasSW) {
