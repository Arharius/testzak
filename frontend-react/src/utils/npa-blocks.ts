...
export function buildSection2Rows(
  ...
  const supportBase: Row2[] = [
    ['2.1. Предмет поставки', 'Поставляется сертификат технической поддержки / продление подписки правообладателя в объёме, достаточном для обслуживания закупаемого программного обеспечения'],
    ['2.2. Документация', 'Поставщик передаёт регламент технической поддержки, описание SLA, порядок регистрации обращений и иные документы правообладателя на русском языке'],
    ['2.3. Активация', 'Поставщик обеспечивает регистрацию сертификата технической поддержки в личном кабинете правообладателя и передачу Заказчику подтверждающих документов'],
  ];
    ...
    if (nacRegime === 'pp1236') {
      restrictionRows = [
        ...
        ['2.5. Подтверждение российского/евразийского происхождения ПО',
          'Участник закупки обязан указать в составе заявки номер реестровой записи программного обеспечения в Едином реестре российского ПО Минцифры России или евразийском реестре ПО (если применимо по ПП РФ № 1875). Правила ведения реестров ПО определяются ПП РФ от 16.11.2015 № 1236 (в действующей редакции)'],
      ];
    ...
    if (nacRegime === 'pp1236') {
      restrictionRows = [
        ...
        ['2.5. Подтверждение включения в реестры ПО',
          'Участник закупки обязан указать в составе заявки номер реестровой записи программного обеспечения в Едином реестре российского ПО Минцифры России или евразийском реестре ПО (если применимо). Правила ведения реестра российского ПО определяются ПП РФ от 16.11.2015 № 1236 (в действующей редакции), реестр используется в том числе для целей применения ПП РФ № 1875'],
      ];
    } else if (nacRegime === 'pp878') {
      restrictionRows = [
  ...
  const baseRows = goodsType === 'osSupport' || goodsType === 'supportCert'
    ? supportBase
    : isSW
      ? swBase
      : hwBase;
