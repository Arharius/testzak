...
function buildRowLegalSummaryText(row: GoodsRow, index: number): string {
  const itemLabel = getRowDisplayLabel(row);
  const okpd2 = getResolvedOkpd2Code(row) || '—';
  const okpd2Name = getResolvedOkpd2Name(row);
  const ktru = getResolvedKtruCode(row);
  ...
  return `Позиция №${index + 1}: ${itemLabel}. ОКПД2: ${okpd2}${okpd2Name ? ` — ${okpd2Name}` : ''}. КТРУ: ${ktru || 'не указан / не применяется'}. ПП РФ № 1875: ${statusLabel}. Источник классификации: ${sourceLabel}.${basisSuffix}${reviewSuffix}`;
}
