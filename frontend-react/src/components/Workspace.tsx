...
• Никаких emoji и символов интерфейса (⭐ 🔴 ✅ ❌ и любых других) в тексте характеристик.
• Никаких комментариев и оговорок: «требуется проверка», «при применимости уточните», «обычно не применяется», «как правило», «по умолчанию» — ТЗ юридический документ, в нём нет места предположениям.
• Никаких пояснений для пользователя системы внутри документа.
• Конкретный производитель без «или эквивалент».
...
async function buildDocx(
  ...
  function buildDocxSectionParas(rows0: SectionTableRow[]): DocxParagraph[] {
    return rows0.map((row) => new Paragraph({
      children: [
        new TextRun({ text: `${row.label}. `, bold: true, font: FONT, size: FONT_SIZE }),
        ...docxRuns(row.value),
      ],
...
Ответ СТРОГО в JSON без пояснений:
{
  ...
  const handleRepairFileSelected = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    showToast('Применяю исправления соответствия 44-ФЗ/223-ФЗ…', true);
    try {
      const formData = new FormData();
      ...
      const fixCount = parseInt(resp.headers.get('X-Compliance-Fixes') ?? '0', 10);
      const blob = await resp.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = file.name.replace(/\.docx$/i, '_исправлен.docx');
      a.style.display = 'none';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(() => URL.revokeObjectURL(url), 5000);
      showToast(fixCount > 0 ? `Готово: исправлено ${fixCount} нарушений соответствия` : 'Готово: документ прошёл проверку без нарушений', true);
    } catch {
