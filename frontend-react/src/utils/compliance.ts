...
const MEASURABLE_NAME_RE = /(колич|объем|объ[её]м|емкост|[её]мкост|размер|ширин|высот|глубин|толщин|мощност|скорост|пропускн|частот|диагонал|разрешен|памят|ядер|поток|срок|верси|уров|класс|ресурс|масса|вес|длин|время реакции|время решения|\bпорт\b|сокет|tbw|mtbf|iops|яркост|контраст|задержк|углы обзора|угол обзора|латентн|ток|напряжен|давлен|температур|угол|радиус|плотност|расстоян|дальност|диапазон|точност|погрешност|чувствит|нагрузк|производительн)/i;
const BOOLEAN_ALLOWED_NAME_RE = /(наличие|поддержка|совместим|интеграц|журналир|аудит|веб-интерфейс|api|экспорт|импорт|консоль|кластеризац|резервн|ролевая модель|двухфактор|авторизац|аутентификац|шлюз|мониторинг|оповещени|доставка|доступ|управление|миграц|политик|сервис|средств|защит|шифрован|контроль|блокиров|регистрац|монтаж|развертыван|разв[её]ртыван|интерфейс|подключение|протокол|клиент|агент|диспетчер|маркировк|очистк|запуск|инструмент|графическ(ое|ие) средст|портретн|pivot|режим поворота)/i;
...
function isConcreteValue(value: string): boolean {
  ...
  // Скорость оптических приводов: 8x, 16x, 24x — нотация «множитель x»
  if (/\b\d+x\b/i.test(normalized)) return true;
  ...
  for (const row of rows) {
    ...
    for (const spec of row.specs) {
      ...
      if (
        ...
        !BOOLEAN_ALLOWED_NAME_RE.test(String(spec.name || '')) &&
        !QUALITATIVE_ALLOWED_NAME_RE.test(String(spec.name || '')) &&
        !/\b\d+x\b/i.test(String(spec.value || ''))
      ) {
        addIssue(
          issues,
          row,
